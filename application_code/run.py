...
try:
    import application_code.kodys_updater as kodys_updater
except ImportError:
    kodys_updater = None

try:
    import application_code.kodys_license as kodys_license
except ImportError:
    kodys_license = None
...
def ExceptHook(excType, excValue, traceObject):
    ...
    try:
        ...
        print("[run.py] WARNING: failed writing to error file: %s" % (errorFile))
...
class MainWindow(QtGui.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowFlags(
            QtCore.Qt.WindowCancelButtonHint | QtCore.Qt.WindowMinimizeButtonHint
        )
    ...
    def closeEvent(self, event):
        ...
        if url.startswith("http://127.0.0.1:5423/APP"):
            reply = QtGui.QMessageBox.question(
                self,
                "Alert",
                "You should Close the test window first to quit application?",
    ...
    try:
        ...
        if ptr_U != None:
            for a_U in range(0, numBytes_U):
                crc_U = (crc_U >> 8) ^ (
                    crcTab16_U[((crc_U) ^ ((ptr_U[a_U]) & 255)) & 255]
                )
    ...
    while True:
        try:
            ...
            if data.strip() == "SERIAL?":
                ...
                browser_func.ExecuteFunction("device_status", "ON")
                thread_connection1 = threading.Thread(
                    target=hcp_blu_device_check_status
                )
                thread_connection1.daemon = True
                thread_connection1.start()
                browser_func.ExecuteFunction("pleasewait_loader_end")
                        ...
                        if len(data) > 16:
                            ...
                            if value[11] == "K" and value[12] == "P":
                                ...
                                if command == 1:
                                    ...
                                    command = 0
                                if command:
                                    browser_func.ExecuteFunction(
                                        "js_print", str(command)
                                    )
                                    browser_func.ExecuteFunction("device_status", "ON")
        except SerialException:
            browser_func.ExecuteFunction("device_status", "OFF")
        ...
        if can_blu_password_ok == False:
            try:
                ...
                if data.strip() == "OK":
                    ...
                    thread_connection1.start()
                    data_time = datetime.datetime.now()
                    thread_connection3 = threading.Thread(
                        target=can_blu_device_status_check
                    )
            ...
            if can_data_type == "PARA":
                try:
                    ...
                    if len(ecg_level_1) > display_len:
                        if can_mode == "diagnose":
                            smooth_data_1 = signal.filtfilt(
                                B, A, ecg_level_1, method="gust"
                            )
                            smooth_data_2 = signal.filtfilt(
                                B, A, ecg_level_2, method="gust"
                            )
                            browser_func.ExecuteFunction(
                                "js_print",
                                str(list(smooth_data_1))
                                .replace("[", "")
                                .replace("]", ""),
                                str(list(smooth_data_2))
                                .replace("[", "")
                                .replace("]", ""),
                                str(ecg_level_3).replace("[", "").replace("]", ""),
                                str(list_data).replace("[", "").replace("]", ""),
                                str(ecg_level_1).replace("[", "").replace("]", ""),
                                str(ecg_level_2).replace("[", "").replace("]", ""),
                            )
                        else:
                            browser_func.ExecuteFunction(
                                "js_print",
                ...
                try:
                    ...
                    if "999" in data:
                        ...
                        browser_func.ExecuteFunction(
                            "js_bp_print", systolic, diastolic, pulse_rate
                        )
                    elif "S0" in data:
                        stop_999_check = True
                        is_error = False
        ...
        if can_password_ok == False:
            try:
                ...
                if response.replace("\x00", "").strip().startswith("SERIAL?"):
                    ...
                    browser_func.ExecuteFunction("device_status", "ON")
                    thread_connection1 = threading.Thread(
                        target=can_device_status_check
                    )
                    thread_connection1.daemon = True
                    thread_connection1.start()
            ...
            if can_data_type == "PARA":
                ...
                if len(ecg_level_1) > display_len:
                    if can_mode == "diagnose":
                        ...
                        browser_func.ExecuteFunction(
                            "js_print",
                            str(list(smooth_data_1)).replace("[", "").replace("]", ""),
                            str(list(smooth_data_2)).replace("[", "").replace("]", ""),
                            str(ecg_level_3).replace("[", "").replace("]", ""),
                            str(list_data).replace("[", "").replace("]", ""),
                            str(ecg_level_1).replace("[", "").replace("]", ""),
                            str(ecg_level_2).replace("[", "").replace("]", ""),
...
def arrhytmia_check():
    ...
    while True:
        if (
            can_data_type == "PARA"
            and can_blu_device_status == "CONNECTED"
            and lead_off == False
        ):
            try:
                current_time = datetime.datetime.now()
...
def usb_arrhytmia_check():
    ...
    while True:
        if (
            ...
            and can_device_status == "CONNECTED"
            and lead_off == False
        ):
            try:
                current_time = datetime.datetime.now()
...
class BluetoothSpp:
    ...
    def gen_enum_key(self, subkey, search_str):
        hKey = winreg.OpenKey(
            winreg.HKEY_LOCAL_MACHINE, self.key_bthenum + "\\" + subkey
        )
    ...
    while True:
        try:
            if os.path.exists(r"C:\\temp\\kodys\\podo_i_mat\\final-output.jpeg"):
                ...
                temp_file = open(
                    assets_media_dir_path
                    + "\\podo_i_mat\\podo_i_mat-"
                    + uid
                    + "."
                    + "jpeg",
                    "wb",
                ...
                browser_func.ExecuteFunction(
                    "load_image",
                    str(
                        "http://127.0.0.1:5423/site_media/podo_i_mat/"
                        + "podo_i_mat-"
                        + uid
                        + "."
                        + "jpeg"
...
def convertIt():
    ...
    dialog.paintRequested.connect(
        lambda p: web_view.page().currentFrame().print_(printer)
    )
    web_view.close()
    dialog.exec_()
                        ...
                        if devices:
                            try:
                                ...
                                QtGui.QMessageBox.question(
                                    widget,
                                    "Alert",
                                    "Unable to connect with device, Please reboot device and try again.",
                                    QtGui.QMessageBox.Ok,
                                )
                        else:
                            self.browser.ExecuteFunction("pleasewait_loader_end")
                            ...
                            QtGui.QMessageBox.question(
                                widget,
                                "Alert",
                                "Please check device USB is connected with PC?",
                                QtGui.QMessageBox.Ok,
                            )
                    else:
                        self.browser.ExecuteFunction("pleasewait_loader_end")
                            ...
                            if port:
                                try:
                                    ...
                                    QtGui.QMessageBox.question(
                                        widget,
                                        "Alert",
                                        "Please check Device is ON and Blutooth is paired with PC?",
                                        QtGui.QMessageBox.Ok,
                                    )
                            else:
                                self.browser.ExecuteFunction("pleasewait_loader_end")
                                ...
                                QtGui.QMessageBox.question(
                                    widget,
                                    "Alert",
                                    "Unable to connect with device,Com port not available!",
                                    QtGui.QMessageBox.Ok,
                                )
                        else:
                            self.browser.ExecuteFunction("pleasewait_loader_end")
                ...
                if str(connection_type) == "USB":
                    ...
                    try:
                        if device_status != "CONNECTED":
                            ...
                            for port in ports:
                                try:
                                    vpt_ser = serial.Serial(
                                        port, 9600, parity="N", write_timeout=3
                                    )
                                    ...
                                    vpt_ser.write(buffer)
                                    thread_connection1 = threading.Thread(
                                        target=data_handler_blu_vpt
                                    )
                                    thread_connection1.daemon = True
                                    thread_connection1.start()
                                    thread_connection = threading.Thread(
                                        target=voltage_enquriy
                                    )
                                    thread_connection.daemon = True
                                    thread_connection.start()
                                    break
                        ...
                        QtGui.QMessageBox.question(
                            widget,
                            "Alert",
                            "Please check Device Blutooth is paired with PC?",
                            QtGui.QMessageBox.Ok,
                        )
                        ...
                        if can_blu_device_status != "CONNECTED":
                            ...
                            if port:
                                try:
                                    ...
                                    can_data_type = ""
                                    stop_999_check = False
                                    thread_connection2 = threading.Thread(
                                        target=data_handler_can_blu,
                                        args=("K" + pass_code_,),
                                    )
                                    thread_connection2.daemon = True
                                    thread_connection2.start()
    ...
    def browser_file(self, js_callback):
        def browser_img_file(side):
            ...
            selected_file = QtGui.QFileDialog.getOpenFileName(
                caption="Open file", filter="Image files (*.jpg *.jpeg *.png)"
            )
            if selected_file:
                ...
                temp_file = open(
                    assets_media_dir_path
                    + "\\podo_i_mat\\podo_i_mat-BrowserImage"
                    + uid
                    + "."
                    + extension,
                    "wb",
        ...
        def save_file(ele):
            try:
                ...
                name = QtGui.QFileDialog.getSaveFileName(
                    caption="Save File", filter="PDF Files (*.pdf *.PDF)"
                )
                import pdfkit
                ...
                pdfkit.from_string(
                    html_string, str(name), configuration=config, options=options
                )
                widget = QtGui.QWidget()
                widget.setWindowTitle(window_title)
                widget.setWindowIcon(QtGui.QIcon(icon_name))
    ...
    def email_pdf(self, js_callback):
        def email_file(ele):
            try:
                ...
                pdfkit.from_string(
                    html_string,
                    assets_dir_path + "\\report\\Report-" + uid + ".pdf",
