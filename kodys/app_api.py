...
# Workaround for newer scipy versions where `scipy.integrate.simps` is no longer exported.
# Some dependencies (e.g., peakutils/biosppy) still import it directly.
try:
    import scipy.integrate as _scipy_integrate

    if not hasattr(_scipy_integrate, "simps"):
        # SciPy >=1.17 renamed simps to simpson. Monkey patch for compatibility.
        if hasattr(_scipy_integrate, "simpson"):
            _scipy_integrate.simps = _scipy_integrate.simpson
except Exception:
    # If scipy isn't installed yet, it will be installed later via requirements.
    pass
...
# numpy 2.0 removed `numpy.trapz` from the top-level namespace.
# Provide a lightweight compatible implementation for trapezoidal integration.
try:
    trapz = np.trapz
    ...
    def trapz(y, x=None, dx=1.0, axis=-1):
        ...
        if x is None:
            ...
            return (
                d
                * (
                    y.take(indices=range(1, y.shape[axis]), axis=axis)
                    + y.take(indices=range(0, y.shape[axis] - 1), axis=axis)
                ).sum(axis=axis)
                / 2.0
            )
        x = np.asanyarray(x)
        ...
        return (d * (y[tuple(slice1)] + y[tuple(slice2)]) / 2.0).sum(axis=axis)


from . import bwr
from . import qrs_detector as qrs_detect
        ...
        if username:
            ...
            if not user:
                if not password:
                    ...
                    result, msg = ulo._setmsg(success_msg, error_msg, False)
                    validation_error = (
                        "There is no account in our system for this Username"
                    )
...
def customer_support(request):
    ...
    user = None
    try:
        customer_support_technical = MA_APPLICATION_CONTACTS.objects.filter(
            POSITION="Technical Support", DATAMODE="A"
        )
        customer_support_marketing = MA_APPLICATION_CONTACTS.objects.filter(
            POSITION="Marketing Support", DATAMODE="A"
        )
        customer_support_product = MA_APPLICATION_CONTACTS.objects.filter(
            POSITION="Product Support", DATAMODE="A"
        )
        customer_support = MA_APPLICATION_CONTACTS.objects.filter(
            POSITION="Customer Support Executives", DATAMODE="A"
        )
...
def home(request):
    ...
    try:
        ...
        for apps in kodys_apps:
            medical_test = TX_MEDICALTESTS.objects.filter(
                MEDICALTESTMASTER__MEDICAL_APP__CODE=apps.CODE, DATAMODE="A"
            ).first()
            if medical_test:
        ...
        for speciality in specialization:
            ...
            doctor = TX_HCP_SPECIALIZATION.objects.filter(
                HCP__TYPE="DOCTOR", SPECIALIZATION=speciality, DATAMODE="A"
            ).order_by("pk")
            doctor_list = list()
            for hcp in doctor:
            ...
            examiners = TX_HCP_SPECIALIZATION.objects.filter(
                HCP__TYPE="EXAMINER", SPECIALIZATION=speciality, DATAMODE="A"
            ).order_by("pk")
            examiner_list = list()
...
def service_email(request, p_dict):
    ...
    try:
        ...
        from_email = MA_APPLICATION_SETTINGS.objects.get(
            KEY_CODE="SET-01", DATAMODE="A"
        ).KEY_VALUE
        server = MA_APPLICATION_SETTINGS.objects.get(
            KEY_CODE="SET-02", DATAMODE="A"
        ).KEY_VALUE
        port = MA_APPLICATION_SETTINGS.objects.get(
            KEY_CODE="SET-03", DATAMODE="A"
        ).KEY_VALUE
        password = MA_APPLICATION_SETTINGS.objects.get(
            KEY_CODE="SET-04", DATAMODE="A"
        ).KEY_VALUE
        to_email = MA_APPLICATION_SETTINGS.objects.get(
            KEY_CODE="SET-05", DATAMODE="A"
        ).KEY_VALUE
        ...
        if doctorname:
            ...
            if TX_HCP.objects.filter(
                ACCOUNTUSER__USER__username=request.user.username
            ).exists():
                hcp = TX_HCP.objects.get(
                    ACCOUNTUSER__USER__username=request.user.username
                )
        ...
        if attachment:
            for attached_file in attachment:
                ...
                with open(
                    "%s/service_request/%s" % (settings.MEDIA_DATA, attached_file),
                    "wb+",
                ) as destination:
                ...
                filename_attach = "%s/service_request/%s" % (
                    settings.MEDIA_DATA,
                    attached_file,
                )
        ...
        if email:
            ...
            if address:
                email_message = "%sHospital Address : %s<br />" % (
                    email_message,
                    address,
                )
            if doctorname:
                email_message = "%sDoctor Name : %s<br />" % (
                    email_message,
                    service_request.C_NAME,
                )
            if service_request.C_MOBILE_NUMBER:
                email_message = "%sDoctor Mobile Number : %s<br />" % (
                    email_message,
                    service_request.C_MOBILE_NUMBER,
                )
            if service_request.C_EMAIL:
                email_message = "%sDoctor Email ID : %s<br />" % (
                    email_message,
                    service_request.C_EMAIL,
                )
            if productname:
                email_message = "%sProduct Name : %s<br />" % (
                    email_message,
                    productname,
                )
            if prblmnature:
                prblmnature = prblmnature.replace("\n", "<br />")
                email_message = "%sNature of Problem :<br />&nbsp;&nbsp;%s" % (
                    email_message,
                    prblmnature,
                )
            if attachment:
            ...
            try:
                ...
                result, msg = ulo._setmsg(
                    success_msg,
                    "Username and Password not accepted.Learn more at https://support.google.com/mail/?p=BadCredentials",
                    False,
                )
                logger.error(ulo.error_log(request, sys.exc_traceback.tb_lineno, e))
    ...
    try:
        ...
        if logo:
            if os.path.isfile("%s/hospital_profile/%s" % (settings.MEDIA_ROOT, logo)):
                ...
                with open(
                    "%s/hospital_profile/%s" % (settings.MEDIA_ROOT, logo), "wb+"
                ) as destination:
        ...
        if username and password:
            ...
            user[0].save()
            if user[1]:
                if TX_ACCOUNTUSER.objects.filter(
                    ACCOUNTROLE__NAME="MANAGER", DATAMODE="A"
                ).exists():
                    accountusers = TX_ACCOUNTUSER.objects.filter(
                        ACCOUNTROLE__NAME="MANAGER", DATAMODE="A"
                    )
                ...
                for account_user in accountusers:
                    ...
                    account_user.DATAMODE = "D"
                    account_user.save()
                new_account_user = TX_ACCOUNTUSER.objects.filter(
                    USER=user[0], DATAMODE="D"
                ).first()
...
def doctors(request, speciality):
    ...
    try:
        ...
        if int(speciality) == 0:
            ...
            hcp_speciality_users = TX_HCP_SPECIALIZATION.objects.filter(
                SPECIALIZATION__id=speciality, DATAMODE="A"
            ).order_by("pk")
            hcp_users = list()
    ...
    try:
        ...
        if hcp_uid:
            ...
            if specialization:
                ...
                specialization = specialization.replace("]", "")
                specialization = specialization.split(",")
                hcp_special = TX_HCP_SPECIALIZATION.objects.filter(
                    HCP=hcp, DATAMODE="A"
                )
                for special in hcp_special:
                ...
                for speciality in specialization:
                    if speciality:
                        speciality = speciality.replace(",", "")
                        if TX_HCP_SPECIALIZATION.objects.filter(
                            HCP=hcp, SPECIALIZATION__NAME=speciality, DATAMODE="A"
                        ).exists():
                            hcp_speialization = TX_HCP_SPECIALIZATION.objects.filter(
                                HCP=hcp, SPECIALIZATION__NAME=speciality, DATAMODE="A"
                            ).first()
                            if hcp_speialization.id in hcp_special_list:
                                hcp_special_list.remove(hcp_speialization.id)
                        elif TX_HCP_SPECIALIZATION.objects.filter(
                            HCP=hcp, SPECIALIZATION__NAME=speciality, DATAMODE="D"
                        ).exists():
                            hcp_speialization = TX_HCP_SPECIALIZATION.objects.filter(
                                HCP=hcp, SPECIALIZATION__NAME=speciality, DATAMODE="D"
                            ).first()
                            ...
                            specialization = MA_HCP_SPECIALIZATION.objects.get(
                                NAME=speciality
                            )
                            hcp_speialization = TX_HCP_SPECIALIZATION()
                            hcp_speialization.SPECIALIZATION = specialization
                ...
                if hcp_special_list:
                    for speciality in hcp_special_list:
                        hcp_speialization = TX_HCP_SPECIALIZATION.objects.get(
                            id=int(speciality), DATAMODE="A"
                        )
...
def hcp_delete(request, hcp_uid):
    ...
    try:
        if hcp_uid:
            ...
            hcp.DATAMODE = "D"
            hcp.save()
            TX_HCP_SPECIALIZATION.objects.filter(HCP=hcp, DATAMODE="A").update(
                DATAMODE="D"
            )
            ...
            if user:
                ...
                if User.objects.filter(username__contains=new_user_name).exists():
                    user_old_name = User.objects.filter(
                        username__contains=new_user_name
                    ).first()
                ...
                if user.email:
                    ...
                    if User.objects.filter(email__contains=new_user_email).exists():
                        user_old_email = User.objects.filter(
                            email__contains=new_user_email
                        ).first()
...
def patients_add(request, p_dict, patient_uid=None):
    ...
    try:
        ...
        patient_prefix = MA_APPLICATION_SETTINGS.objects.get(
            KEY_CODE="SET-07", DATAMODE="A"
        )
        digits = MA_APPLICATION_SETTINGS.objects.get(
            KEY_CODE="SET-08", DATAMODE="A"
        ).KEY_VALUE
        ...
        if not patients.FRIENDLY_UID:
            if patient_id:
                ...
                if patient_prefix.KEY_VALUE and digits:
                    new_prefix = "%s%s" % (
                        patient_prefix.KEY_VALUE,
                        str(1).zfill(int(digits)),
                    )
                    if TX_PATIENTS.objects.filter(FRIENDLY_UID=new_prefix).exists():
                        exist_patient = TX_PATIENTS.objects.filter(
                            FRIENDLY_UID__icontains=patient_prefix.KEY_VALUE,
                            PREFIX_DIGITS_LENGTH=int(digits),
                        ).first()
                        prefix_length = exist_patient.FRIENDLY_UID.replace(
                            "%s" % (patient_prefix.KEY_VALUE), ""
                        )
                        if int(len(prefix_length)) == int(digits):
                            prefix_length = prefix_length.replace("0", "")
                            new_prefix = "%s%s" % (
                                patient_prefix.KEY_VALUE,
                                str(int(prefix_length) + 1).zfill(int(digits)),
                            )
                    else:
                        new_prefix = "%s%s" % (
                            patient_prefix.KEY_VALUE,
                    ...
                    if TX_PATIENTS.objects.filter(FRIENDLY_UID=new_prefix).exists():
                        exist_patient = TX_PATIENTS.objects.filter(
                            FRIENDLY_UID__icontains="PAT", PREFIX_DIGITS_LENGTH=int(4)
                        ).first()
                        prefix_length = exist_patient.FRIENDLY_UID.replace(
                            "%s" % ("PAT"), ""
                        )
                        if int(len(prefix_length)) == int(4):
                            ...
                            new_prefix = "%s%s" % (
                                "PAT",
                                str(int(prefix_length) + 1).zfill(int(4)),
                            )
                    patients.FRIENDLY_UID = new_prefix
...
def reports(request, speciality, duration):
    ...
    try:
        ...
        if speciality == "All":
            if duration:
                ...
                if str(accountuser.ACCOUNTROLE) == "MEMBER":
                    reports = TX_MEDICALTESTREPORTS.objects.filter(
                        REPORTED_ON__range=[start_date.date(), end_date.date()],
                        DATAMODE="A",
                        CREATED_BY=accountuser.USER,
            ...
            if duration:
                ...
                if str(accountuser.ACCOUNTROLE) == "MEMBER":
                    reports = TX_MEDICALTESTREPORTS.objects.filter(
                        MEDICALTEST__MEDICALTESTMASTER__MEDICAL_APP__CODE=speciality,
                        REPORTED_ON__range=[start_date.date(), end_date.date()],
                        DATAMODE="A",
                        CREATED_BY=accountuser.USER,
                    ).order_by("-REPORTED_ON")
        ...
        for report in reports:
            report_dict = dict()
            report_dict["REPORTED_ON"] = report.REPORTED_ON.strftime(
                "%d/%m/%Y %I:%M %p"
            )
            report_dict["PATIENT_UID"] = report.MEDICALTEST.PATIENT.FRIENDLY_UID
            report_dict["PATIENT_NAME"] = "%s %s %s" % (
                report.MEDICALTEST.PATIENT.NAME,
                report.MEDICALTEST.PATIENT.LAST_NAME,
                report.MEDICALTEST.PATIENT.SUR_NAME,
            )
            report_dict["PATIENT_PREFIX"] = (
                report.MEDICALTEST.PATIENT.PREFIX.capitalize()
            )
            if report.MEDICALTEST.DOCTOR:
                report_dict["DOCTOR_NAME"] = report.MEDICALTEST.DOCTOR.NAME
                report_dict["DOCTOR_PREFIX"] = (
                    report.MEDICALTEST.DOCTOR.PREFIX.capitalize()
                )
            ...
            if report.MEDICALTEST.EXAMINER:
                ...
                report_dict["EXAMINER_PREFIX"] = (
                    report.MEDICALTEST.EXAMINER.PREFIX.capitalize()
                )
            ...
            if (
                report.MEDICALTEST.MEDICALTESTMASTER.ORGAN_TYPE
                and report.MEDICALTEST.MEDICALTESTMASTER.ORGAN_TYPE.lower() == "hand"
            ):
                report_dict["MEDICALTEST"] = "%s - %s" % (
                    report.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.APP_NAME,
                    report.MEDICALTEST.MEDICALTESTMASTER.ORGAN_TYPE,
                )
            elif (
                report.MEDICALTEST.MEDICALTESTMASTER.ORGAN_TYPE
                and report.MEDICALTEST.MEDICALTESTMASTER.ORGAN_TYPE.lower() == "other"
            ):
                report_dict["MEDICALTEST"] = "%s - %s" % (
                    report.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.APP_NAME,
                    report.MEDICALTEST.MEDICALTESTMASTER.ORGAN_TYPE,
                ...
                if (
                    report.TEST_TYPE
                    and report.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.CODE
                    == "APP-01"
                ):
                    if report.MEDICALTEST.MEDICALTESTMASTER.CODE == "TM-01":
                        ...
                        report_dict["MEDICALTEST"] = "%s - %s" % (
                            report.MEDICALTEST.MEDICALTESTMASTER.TEST_NAME,
                            report.TEST_TYPE.replace("_", " "),
                        )
                elif report.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.CODE == "APP-07":
                    if report.TEST_TYPE == "All_test":
                        report_dict["MEDICALTEST"] = "%s - All" % (
                            report.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.APP_NAME.replace(
                                "Test", ""
                            ).strip()
                        )
                    elif report.TEST_TYPE == "hrv_test":
                        report_dict["MEDICALTEST"] = "%s - Hrv" % (
                            report.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.APP_NAME.replace(
                                "Test", ""
                            ).strip()
                        )
                    else:
                        report_dict["MEDICALTEST"] = "%s - Parasympathetic" % (
                            report.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.APP_NAME.replace(
                                "Test", ""
                            ).strip()
                        )
                elif report.TEST_TYPE:
                    report_dict["MEDICALTEST"] = "%s - %s - %s" % (
                        report.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.APP_NAME,
                        report.MEDICALTEST.MEDICALTESTMASTER.ORGAN_TYPE,
                        report.TEST_TYPE,
                    )
            ...
            if report.MEDICALTEST.MEDICALTESTMASTER.ORGAN_TYPE:
                report_dict["TEST_TYPE"] = (
                    report.MEDICALTEST.MEDICALTESTMASTER.ORGAN_TYPE.lower()
                )
            # elif report.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.CODE == "APP-07":
            #   report_dict['TEST_TYPE'] = report.TEST_TYPE
            else:
                report_dict["TEST_TYPE"] = ""
            TEST_CODE = TX_MEDICALTESTS.objects.filter(
                FRIENDLY_UID=report.MEDICALTEST.FRIENDLY_UID, DATAMODE="A"
            ).first()
            ...
            report_dict["APP_CODE"] = (
                report.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.CODE
            )
...
def report_view(request, test_entry_code):
    ...
    REPORT_PATH = None
    try:
        reports = TX_MEDICALTESTREPORTS.objects.get(
            MEDICALTEST__FRIENDLY_UID=test_entry_code, DATAMODE="A"
        )
        ...
        medical_test = TX_MEDICALTESTS.objects.filter(
            FRIENDLY_UID=test_entry_code, DATAMODE="A"
        ).order_by("pk")
        if medical_test:
            # if medical_test[0].MEDICALTESTMASTER.MEDICAL_APP.CODE == "APP-04":
            if medical_test[0].MEDICALTESTMASTER.CODE == "TM-07":
                report["medical_test_organ_type"] = medical_test[
                    0
                ].MEDICALTESTMASTER.ORGAN_TYPE
            ...
            for medical_test_value in medical_test:
                tx_test_entries = TX_MEDICALTESTENTRIES.objects.filter(
                    MEDICALTEST__id=medical_test_value.id, DATAMODE="A"
                )
                report[medical_test_value.MEDICALTESTMASTER.CODE.replace("-", "")] = (
                    tx_test_entries
                )
                try:
                    if medical_test[0].MEDICALTESTMASTER.MEDICAL_APP.CODE != "APP-07":
                        medical_test_interpertation = MA_MEDICALTESTMASTERINTERPERTATION.objects.get(
                            MEDICALTESTMASTER__CODE=medical_test_value.MEDICALTESTMASTER.CODE,
                            DATAMODE="A",
                        )
                        report["medical_test"] = medical_test_value
                        if (
                            medical_test_interpertation.MEDICALTESTMASTER.CODE
                            == "TM-13"
                        ):
                            for (
                                key,
                                value,
                            ) in medical_test_interpertation.RANGES.iteritems():
                                report[str(key.replace(" ", "_"))] = [str(key), value]
                        else:
                            report["medical_test_interpertation"] = (
                                medical_test_interpertation
                            )
                    elif medical_test[0].MEDICALTESTMASTER.MEDICAL_APP.CODE == "APP-07":
                        raw_data = (
                            report[
                                medical_test_value.MEDICALTESTMASTER.CODE.replace(
                                    "-", ""
                                )
                            ][2].KEY_VALUE
                        ).replace(" ", "")
                        ...
                        smooth_data_list = signal.filtfilt(B, A, data_list)
                        ...
                        if medical_test_value.MEDICALTESTMASTER.CODE == "TM-21":
                            ...
                            report[
                                ...
                                + "_standing_processed_data"
                            ] = ecg_out_string
                        report["medical_test"] = medical_test_value
                        interpertation_dict = dict()
                        for interpertation in medical_test:
                            if interpertation.MEDICALTESTMASTER.CODE != "TM-25":
                                test_interpertation = MA_MEDICALTESTMASTERINTERPERTATION.objects.get(
                                    MEDICALTESTMASTER__CODE=interpertation.MEDICALTESTMASTER.CODE,
                                    DATAMODE="A",
                                )
                                interpertation_dict[
                                    interpertation.MEDICALTESTMASTER.CODE.replace(
                                        "-", ""
                                    )
                                ] = test_interpertation.RANGES
...
def graph_data(request, workbook, key_type, reports, key_data, test_code):
    fn = ulo._fn()
    logger.info(ulo.start_log(request, fn))
    rrn_list = list()
    heart_beat_list = list()
    rr_interval = list()
    length = 0
    try:
        if "rrn" in key_data and "rrn_plus1" in key_data:
            length = (
                len(key_data["rrn"])
                if len(key_data["rrn"]) == len(key_data["rrn_plus1"])
                else 0
            )
            for rrn_value in range(0, length):
                rrn_list.append(
                    (key_data["rrn"][rrn_value], key_data["rrn_plus1"][rrn_value])
                )
            rrn_worksheet = workbook.add_worksheet(
                "%s %s RRNRATIO" % (test_code, key_type.upper())
            )
            rrn_worksheet.write(0, 0, "RRN")
            rrn_worksheet.write(0, 1, "RRNPLUS1")
            row, col = 1, 0
            for rrn, rrn_plus1 in rrn_list:
                rrn_worksheet.write(row, col, rrn)
                rrn_worksheet.write(row, col + 1, rrn_plus1)
                row += 1
        if "heart_rate_graph" in key_data:
            heart_beat_worksheet = workbook.add_worksheet(
                "%s %s HEARTBEAT" % (test_code, key_type.upper())
            )
            heart_beat_worksheet.write(0, 0, "HEARTBEAT")
            heart_beat_worksheet.write(0, 1, "COUNT")
            row, col = 1, 0
            for heart_beat_count in key_data["heart_rate_graph"]:
                heart_beat_worksheet.write(row, col, heart_beat_count[0])
                heart_beat_worksheet.write(row, col + 1, heart_beat_count[1])
                row += 1
        if "rr_interval" in key_data:
            rr_interval_worksheet = workbook.add_worksheet(
                "%s %s RRINTERVAL" % (test_code, key_type.upper())
            )
            rr_interval_worksheet.write(0, 0, "RRINTERVAL")
            row, col = 1, 0
            for rr_interval in key_data["rr_interval"]:
                rr_interval_worksheet.write(row, col, rr_interval)
                row += 1
    except Exception as e:
        logger.error(ulo.error_log(request, sys.exc_traceback.tb_lineno, e))
    logger.info(ulo.end_log(request, fn))
...
def ecg_raw_lead_data(request, workbook, test_code, test_name, data_type, data_list):
    fn = ulo._fn()
    logger.info(ulo.start_log(request, fn))
    try:
        data_worksheet = workbook.add_worksheet(
            "%s %s %s" % (test_code, test_name, data_type)
        )
        data_worksheet.write(0, 0, "%s" % (data_type))
        row, col = 1, 0
        for data in data_list:
            if data:
                data_worksheet.write(row, col, int(data))
                row += 1
    except Exception as e:
        logger.error(ulo.error_log(request, sys.exc_traceback.tb_lineno, e))
    logger.info(ulo.end_log(request, fn))
...
def canyscope_report_export(request, test_entry_code):
    fn = ulo._fn()
    logger.info(ulo.start_log(request, fn))
    resp = ""
    try:
        ...
        report_result = json.loads(reports.TEST_RESULT)
        s = io.StringIO()
        workbook = xlsxwriter.Workbook(s)
        if "resting" in report_result:
            graph_data(
                request,
                workbook,
                "RESTING",
                reports,
                report_result["resting"],
                "TM-15",
            )
        if "deep_breathing_time" in report_result:
            graph_data(
                request,
                workbook,
                "DEEP BREATHING",
                reports,
                report_result["deep_breathing_time"],
                "TM-20",
            )
        if "ecg_standing_time" in report_result:
            graph_data(
                request,
                workbook,
                "STANDING",
                reports,
                report_result["ecg_standing_time"],
                "TM-21",
            )
        if "ecg_valsalva_time" in report_result:
            graph_data(
                request,
                workbook,
                "VALSALVA",
                reports,
                report_result["ecg_valsalva_time"],
                "TM-22",
            )
        if "hrv_time" in report_result:
            graph_data(
                request, workbook, "HRV", reports, report_result["hrv_time"], "TM-25"
            )
        ...
        if medical_test:
            for medical_test_value in medical_test:
                raw_data_list, lead1_data_list, lead2_data_list, lead3_data_list = (
                    list(),
                    list(),
                    list(),
                    list(),
                ...
                try:
                    if medical_test[0].MEDICALTESTMASTER.MEDICAL_APP.CODE == "APP-07":
                        if medical_test_value.MEDICALTESTMASTER.CODE not in [
                            "TM-23",
                            "TM-24",
                        ]:
                            test_name = (
                                medical_test_value.MEDICALTESTMASTER.TEST_NAME.split(
                                    "ECG - "
                                )[-1]
                            )
                            test_code = medical_test_value.MEDICALTESTMASTER.CODE
                            if medical_test_value.MEDICALTESTMASTER.CODE == "TM-21":
                                if tx_test_entries[4].KEY_VALUE:
                                    raw_data = (tx_test_entries[4].KEY_VALUE).replace(
                                        " ", ""
                                    )
                                ...
                                if tx_test_entries[5].KEY_VALUE:
                                    lead1_data = (tx_test_entries[5].KEY_VALUE).replace(
                                        " ", ""
                                    )
                                    lead1_data_list = lead1_data.split(",")
                                    ecg_raw_lead_data(
                                        request,
                                        workbook,
                                        test_code,
                                ...
                                if tx_test_entries[6].KEY_VALUE:
                                    lead2_data = (tx_test_entries[6].KEY_VALUE).replace(
                                        " ", ""
                                    )
                                    lead2_data_list = lead2_data.split(",")
                                    ecg_raw_lead_data(
                                        request,
                                        workbook,
                                        test_code,
                                ...
                                if tx_test_entries[7].KEY_VALUE:
                                    lead3_data = (tx_test_entries[7].KEY_VALUE).replace(
                                        " ", ""
                                    )
                                    lead3_data_list = lead3_data.split(",")
                                    ecg_raw_lead_data(
                                        request,
                                        workbook,
                                        test_code,
                                ...
                                if medical_test_value.MEDICALTESTMASTER.CODE == "TM-22":
                                    test_name = "VALSALVA"
                                if medical_test_value.MEDICALTESTMASTER.CODE == "TM-25":
                                    test_name = "HRV"
                                if tx_test_entries[0].KEY_VALUE:
                                    raw_data = (tx_test_entries[0].KEY_VALUE).replace(
                                        " ", ""
                                    )
                                    raw_data_list = raw_data.split(",")
                                    ecg_raw_lead_data(
                                ...
                                if tx_test_entries[1].KEY_VALUE:
                                    lead1_data = (tx_test_entries[1].KEY_VALUE).replace(
                                        " ", ""
                                    )
                                    lead1_data_list = lead1_data.split(",")
                                    ecg_raw_lead_data(
                                ...
                                if tx_test_entries[2].KEY_VALUE:
                                    lead2_data = (tx_test_entries[2].KEY_VALUE).replace(
                                        " ", ""
                                    )
                                    lead2_data_list = lead2_data.split(",")
                                    ecg_raw_lead_data(
                                ...
                                if tx_test_entries[3].KEY_VALUE:
                                    lead3_data = (tx_test_entries[3].KEY_VALUE).replace(
                                        " ", ""
                                    )
                                    lead3_data_list = lead3_data.split(",")
                                    ecg_raw_lead_data(
        ...
        resp["Content-Disposition"] = "attachment; filename=%s_%s_%s_ecgdata.xlsx" % (
            reports.FRIENDLY_UID,
            reports.MEDICALTEST.PATIENT.FRIENDLY_UID,
            reports.MEDICALTEST.PATIENT.NAME,
        )
...
def app_configuration(request):
    ...
    try:
        if TX_ACCOUNT.objects.filter(DATAMODE="A").exists():
            account = TX_ACCOUNT.objects.get(DATAMODE="A")
            if TX_ACCOUNTLICENSE.objects.filter(
                ACCOUNT__BUSINESS_NAME=account.BUSINESS_NAME, DATAMODE="A"
            ).exists():
                license = TX_ACCOUNTLICENSE.objects.get(
                    ACCOUNT__BUSINESS_NAME=account.BUSINESS_NAME, DATAMODE="A"
                )
            else:
                license = ""
        ...
        if TX_DATABASEBACKUPLOGS.objects.filter(DATAMODE="A").exists():
            ...
            app_config_dict["backup"] = backup.CREATED_ON.strftime(
                "%B %d, %Y at %I:%M %p"
            )
        else:
            app_config_dict["backup"] = "No Backup for Database"

        app_config_details = MA_APPLICATION_SETTINGS.objects.filter(
            DATAMODE="A"
        ).order_by("pk")
        for app_details in app_config_details:
            if app_details.KEY_VALUE:
                ...
                if app_details.KEY_NAME not in app_config_dict.keys():
                    ...
                    if (
                        app_details.KEY_CODE == "SET-09"
                        or app_details.KEY_CODE == "SET-10"
                        or app_details.KEY_CODE == "SET-11"
                    ):
                        app_config_dict[app_details.KEY_NAME] = (
                            json.loads(app_details.KEY_VALUE),
                            app_details.KEY_CODE,
                        )
                    else:
                        app_config_dict[app_details.KEY_NAME] = (
                            app_details.KEY_VALUE,
                            app_details.KEY_CODE,
                        )
                else:
                    if (
...
def manuals(request, category):
    ...
    manuals_list = list()
    try:
        manuals_list = MA_APPLICATION_MANUALS.objects.filter(
            CATEGORY=category, DATAMODE="A"
        ).order_by("pk")
        result, msg = ulo._setmsg(success_msg, error_msg, True)
...
def database_backup(request):
    ...
    try:
        ...
        if not password:
            password = settings.EMAIL_HOST_PASSWORD
        destination_file = "%s/database_backup/db_%s.json" % (
            settings.MEDIA_DATA,
            datetime.datetime.now().strftime("%d_%m_%Y_%I_%M"),
        )
        destination_filename = destination_file.replace(
            "%s/" % (settings.MEDIA_DATA), settings.DATA_URL
        )
        try:
            process = subprocess.Popen(
                "python manage.py dumpdata > %s" % (destination_file),
                shell=True,
            ...
            if TX_ACCOUNT.objects.filter(DATAMODE="A").exists():
                ...
                msg = EmailMessage(
                    "DB Backup", message, from_email, [to_email], connection=backend
                )
                msg.content_subtype = "html"
...
def restore_database(request):
    ...
    try:
        ...
        if source_file:
            ...
            copyfile(src, dst)
            source_file_path = "%s/database_backup/%s" % (
                settings.MEDIA_DATA,
                source_file,
            )
            process = subprocess.Popen(
                "python manage.py makemigrations", shell=True, stdout=subprocess.PIPE
            )
            process = subprocess.Popen(
            ...
            process = subprocess.Popen(
                "python manage.py loaddata %s" % (source_file_path),
                shell=True,
                stdout=subprocess.PIPE,
            )
            process.wait()
...
def app_license(request, p_dict):
    ...
    try:
        if "license" in p_dict and p_dict["license"]:
            ...
            if license_file:
                output_license_file = "%s_%s.kod" % (
                    "license",
                    datetime.datetime.now().strftime("%d_%m_%Y_%I_%M"),
                )
                license_data = license_file.read()
                if os.path.isfile(
                    "%s/hospital_license/%s" % (settings.MEDIA_ROOT, license_file)
                ):
                    pass
                else:
                    with open(
                        "%s/hospital_license/%s"
                        % (settings.MEDIA_DATA, output_license_file),
                        "wb+",
                    ) as destination:
                ...
                try:
                    ...
                    if TX_ACCOUNT.objects.filter(
                        BUSINESS_NAME=decrypted["hospital_name"].strip(), DATAMODE="A"
                    ).exists():
                        account = TX_ACCOUNT.objects.get(
                            BUSINESS_NAME=decrypted["hospital_name"].strip(),
                            DATAMODE="A",
                        )
                    else:
                    ...
                    account.save()
                    if decrypted["TEST"]:
                        MA_MEDICALAPPS.objects.exclude(CODE="APP-08").all().update(
                            DATAMODE="D"
                        )
                    ...
                    if TX_ACCOUNTLICENSE.objects.filter(DATAMODE="A").exists():
                        TX_ACCOUNTLICENSE.objects.filter(DATAMODE="A").update(
                            DATAMODE="D"
                        )
...
def app_configuration_settings(request, p_dict):
    ...
    try:
        if "From" in p_dict and p_dict["From"]:
            ...
            mail_settings = MA_APPLICATION_SETTINGS.objects.get(
                KEY_NAME="From", DATAMODE="A"
            )
...
def search(request, search_key):
    ...
    try:
        patients = TX_PATIENTS.objects.filter(
            NAME__icontains=search_key, DATAMODE="A"
        ).order_by("pk")
        for patient in patients:
            patient_dict = dict()
            patient_dict["NAME"] = "%s %s %s (%s)" % (
                patient.NAME,
                patient.LAST_NAME,
                patient.SUR_NAME,
                patient.FRIENDLY_UID,
            )
            patient_dict["FRIENDLY_UID"] = patient.FRIENDLY_UID
            patient_dict["GENDER"] = patient.GENDER.capitalize()
...
def medical_test_entry(request, p_dict):
    ...
    try:
        ...
        referred_by_medical_test_type = MA_MEDICALTESTMASTER.objects.filter(
            MEDICAL_APP__CODE=app_code, DATAMODE="A"
        ).last()
        if test_code:
            medical_test_type = MA_MEDICALTESTMASTER.objects.filter(
                CODE=test_code, DATAMODE="A"
            ).first()
            test_interpertation = MA_MEDICALTESTMASTERINTERPERTATION.objects.get(
                MEDICALTESTMASTER__CODE=test_code, DATAMODE="A"
            )
            ...
            if (
                test_entry
                and TX_MEDICALTESTS.objects.filter(
                    FRIENDLY_UID=test_entry, MEDICALTESTMASTER__CODE=test_code
                ).exists()
            ):
                medical_test_fields = TX_MEDICALTESTENTRIES.objects.filter(
                    MEDICALTEST__FRIENDLY_UID=test_entry,
                    MEDICALTEST__MEDICALTESTMASTER__CODE=test_code,
                )
                for test_fields in medical_test_fields:
                    ...
                    if test_field_value:
                        ...
                        elif float(test_field_value) >= float(normal[0]) and float(
                            test_field_value
                        ) <= float(normal[1]):
                ...
                medical_test = TX_MEDICALTESTS.objects.filter(
                    FRIENDLY_UID=test_entry, DATAMODE="A"
                ).first()
                ...
                medical_test_fields = MA_MEDICALTESTFIELDS.objects.filter(
                    MEDICALTESTMASTER=medical_test_type, DATAMODE="A"
                )
                for test_fields in medical_test_fields:
            ...
            if TX_MEDICALTESTREPORTS.objects.filter(
                MEDICALTEST__FRIENDLY_UID=medical_test.FRIENDLY_UID, DATAMODE="A"
            ).exists():
                pass
            else:
            ...
            if "test_completed_list" in request.session:
                ...
                request.session["test_completed_list"] = list()
                request.session["test_completed_list"].append(test_code)
            request.session["test_completed_list_length"] = len(
                request.session["test_completed_list"]
            )
            medical_test_result["test_entry_id"] = medical_test.FRIENDLY_UID
            medical_test_result["current_test_code"] = (
                medical_test.MEDICALTESTMASTER.CODE
            )
...
def test_impression(request, p_dict):
    ...
    try:
        ...
        if test_entry and notes:
            medical_test_report = TX_MEDICALTESTREPORTS.objects.get(
                MEDICALTEST__FRIENDLY_UID=test_entry, DATAMODE="A"
            )
            if (
                medical_test_report.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.CODE
                == "APP-07"
            ):
                if medical_test_report.TEST_TYPE == "All_test":
                    medical_tests = TX_MEDICALTESTS.objects.filter(
                        FRIENDLY_UID=test_entry, DATAMODE="A"
                    ).first()
                    if medical_tests.MEDICALTESTMASTER.CODE == "TM-24":
                        ...
                        medical_tests.MEDICALTESTMASTER.CODE == "TM-25"
                        and medical_test_report.IMPRESSION_NOTES
                    ):
...
def thermocool_foot(request, option):
    ...
    try:
        ...
        if option["test_code"]:
            medical_test_type = MA_MEDICALTESTMASTER.objects.get(
                CODE=option["test_code"], DATAMODE="A"
            )
        else:
            medical_test_type = MA_MEDICALTESTMASTER.objects.filter(
                MEDICAL_APP=kodys_apps, DATAMODE="A"
            ).last()
        ...
        medical_device = MA_MEDICALAPPDEVICES.objects.get(
            MEDICAL_APP=kodys_apps, DATAMODE="A"
        )
        if TX_MEDICALDEVICESTATUS.objects.filter(
            MEDICAL_DEVICE=medical_device, DATAMODE="A"
        ).exists():
            device_details = TX_MEDICALDEVICESTATUS.objects.filter(
                MEDICAL_DEVICE=medical_device, DATAMODE="A"
            ).first()
        else:
            device_details = dict()
        ...
        test_interpertation = MA_MEDICALTESTMASTERINTERPERTATION.objects.get(
            MEDICALTESTMASTER__CODE=medical_test_type.CODE, DATAMODE="A"
        )
        normal = str(test_interpertation.RANGES["Normal"])
        abnormal = str(test_interpertation.RANGES["Abnormal"])
        ...
        if option["test_entry"]:
            ...
            medical_test_completed_list = TX_MEDICALTESTS.objects.filter(
                FRIENDLY_UID=option["test_entry"], DATAMODE="A"
            ).order_by("pk")
            for completed_list in medical_test_completed_list:
                medical_test["test_completed_list"].append(
                    completed_list.MEDICALTESTMASTER.CODE
                )
                if "test_completed_list" in request.session:
                    if (
                        completed_list.MEDICALTESTMASTER.CODE
                        in request.session["test_completed_list"]
                    ):
                        pass
                    else:
                        request.session["test_completed_list"].append(
                            completed_list.MEDICALTESTMASTER.CODE
                        )
            request.session["test_completed_list_length"] = len(
                request.session["test_completed_list"]
            )
            medical_test["test_completed_list_length"] = len(
                medical_test_completed_list
            )
            test_report = TX_MEDICALTESTREPORTS.objects.get(
                MEDICALTEST__FRIENDLY_UID=option["test_entry"], DATAMODE="A"
            )
        ...
        if option["test_code"] and option["test_entry"]:
            tx_test_entries = TX_MEDICALTESTENTRIES.objects.filter(
                MEDICALTEST__FRIENDLY_UID=option["test_entry"],
                MEDICALTEST__MEDICALTESTMASTER__CODE=option["test_code"],
                DATAMODE="A",
            )
            if tx_test_entries:
            ...
            if (
                request.session["test_type"] == "Foot"
                and request.session["test_for"] == "All"
                and len(request.session["test_completed_list"]) == 4
            ):
                medical_tests = TX_MEDICALTESTS.objects.filter(
                    FRIENDLY_UID=option["test_entry"], DATAMODE="A"
                )
                ...
                request.session["test_type"] == "Foot"
                and request.session["test_for"] == "Warm"
                and len(request.session["test_completed_list"]) == 2
            ):
                medical_tests = TX_MEDICALTESTS.objects.filter(
                    FRIENDLY_UID=option["test_entry"], DATAMODE="A"
                )
            ...
            if medical_tests:
                ...
                for medical_test_value in medical_tests:
                    ...
                    if right_count != 0 and right_avg:
                        ...
                        if float(right_avg) >= float(normal[0]) and float(
                            right_avg
                        ) <= float(normal[1]):
                            ...
                            right_hand_result = "Not in Range"
                        hcp_test_values[medical_test_value.MEDICALTESTMASTER.CODE][
                            "right_foot_result"
                        ] = ["%.1f" % (right_avg), right_hand_result]
                    else:
                        hcp_test_values[medical_test_value.MEDICALTESTMASTER.CODE][
                            "right_foot_result"
                        ] = ["No Data", ""]
                    ...
                    if left_count != 0 and left_avg:
                        ...
                        if float(left_avg) >= float(normal[0]) and float(
                            left_avg
                        ) <= float(normal[1]):
                ...
                report = TX_MEDICALTESTREPORTS.objects.get(
                    MEDICALTEST__FRIENDLY_UID=option["test_entry"], DATAMODE="A"
                )
    ...
    try:
        ...
        if option["test_code"] and option["test_entry"]:
            ...
            if tx_test_entries:
                ...
                medical_test["test_entry_data"] = ""
            if (
                request.session["test_type"] == "Hand"
                and len(request.session["test_completed_list"]) == 2
            ):
                medical_tests = TX_MEDICALTESTS.objects.filter(
                    FRIENDLY_UID=option["test_entry"], DATAMODE="A"
    ...
    try:
        ...
        if option["test_code"] and option["test_entry"]:
            ...
            if tx_test_entries:
                ...
                medical_test["test_entry_data"] = ""
            if (
                request.session["test_type"] == "Other"
                and len(request.session["test_completed_list"]) == 2
            ):
                medical_tests = TX_MEDICALTESTS.objects.filter(
                    FRIENDLY_UID=option["test_entry"], DATAMODE="A"
                ...
                for medical_test_value in medical_tests:
                    ...
                    if other_count != 0 and other_avg:
                        ...
                        if float(other_avg) >= float(normal[0]) and float(
                            other_avg
                        ) <= float(normal[1]):
                            other_result = "Normal"
                        elif "<" in abnormal:
                ...
                for test_fields in medical_test_fields:
                    ...
                    if test_fields.MEDICALTESTFIELDS.KEY_CODE in extra_list:
                        if "F" in test_fields.KEY_CODE:
                            ...
                            test_fields.KEY_NAME = p_dict.get(KEY_NAME, "")
                        test_fields.MEDICALTESTFIELDS.KEY_NAME = p_dict.get(
                            KEY_NAME, ""
                        )
                        test_fields.MEDICALTESTFIELDS.save()
                    if test_field_value:
                        ...
                        if test_field_value.isalpha():
                            ...
                            test_fields.KEY_VALUE_STATUS = "Normal"
                        elif float(test_field_value) >= float(mild[0]) and float(
                            test_field_value
                        ) <= float(mild[1]):
                            test_fields.KEY_VALUE_STATUS = "Mild"
                        elif float(test_field_value) >= float(severe[0]) and float(
                            test_field_value
                        ) <= float(severe[1]):
                            test_fields.KEY_VALUE_STATUS = "Severe"
                        elif float(test_field_value) >= float(moderate[0]) and float(
                            test_field_value
                        ) <= float(moderate[1]):
            ...
            if medical_tests:
                ...
                for medical_test_value in medical_tests:
                    ...
                    if right_count != 0 and right_avg:
                        ...
                        elif float(right_avg) >= float(mild[0]) and float(
                            right_avg
                        ) <= float(mild[1]):
                            right_foot_result = "Mild"
                        elif float(right_avg) >= float(severe[0]) and float(
                            right_avg
                        ) <= float(severe[1]):
                            right_foot_result = "Severe"
                        elif float(right_avg) >= float(moderate[0]) and float(
                            right_avg
                        ) <= float(moderate[1]):
                        ...
                        vpt_ultra_test_values["right_foot_result"] = [
                            "%.1f" % (right_avg),
                            right_foot_result,
                        ]
                    else:
                    ...
                    if left_count != 0 and left_avg:
                        ...
                        if float(left_avg) >= float(normal[0]) and float(
                            ...
                            left_foot_result = "Normal"
                        elif float(left_avg) >= float(mild[0]) and float(
                            left_avg
                        ) <= float(mild[1]):
                            left_foot_result = "Mild"
                        elif float(left_avg) >= float(severe[0]) and float(
                            left_avg
                        ) <= float(severe[1]):
                            left_foot_result = "Severe"
                        elif float(left_avg) >= float(moderate[0]) and float(
                            left_avg
                        ) <= float(moderate[1]):
                            ...
                            left_foot_result = "Not in Range"
                        vpt_ultra_test_values["left_foot_result"] = [
                            "%.1f" % (left_avg),
                            left_foot_result,
                        ]
                    else:
                        ...
                        if float(patient.HEIGHT) and float(patient.WEIGHT):
                            ...
                            bmi_avg = round(
                                float(
                                    ...
                                    / (
                                        (float(patient.HEIGHT) / 100)
                                        * (float(patient.HEIGHT) / 100)
                                    )
                                ),
                                2,
                            )
                            if float(bmi_avg) >= float(bmi_normal[0]) and float(
                                bmi_avg
                            ) <= float(bmi_normal[1]):
                                bmi_result = "Normal"
                            elif float(bmi_avg) >= float(bmi_over_weight[0]) and float(
                                bmi_avg
                            ) <= float(bmi_over_weight[1]):
                    ...
                    if tx_test_entries[20].KEY_VALUE and tx_test_entries[22].KEY_VALUE:
                        if (
                            tx_test_entries[20].KEY_VALUE.isalpha()
                            or tx_test_entries[22].KEY_VALUE.isalpha()
                        ):
                            vpt_ultra_test_values["right_abi_result"] = ["No Data", ""]
                        else:
                            if float(tx_test_entries[20].KEY_VALUE) and float(
                                tx_test_entries[22].KEY_VALUE
                            ):
                                right_abi = round(
                                    float(
                                        float(tx_test_entries[20].KEY_VALUE)
                                        / float(tx_test_entries[22].KEY_VALUE)
                                    ),
                                    2,
                                )
                                if float(right_abi) >= float(abi_severe[0]) and float(
                                    right_abi
                                ) <= float(abi_severe[1]):
                                    right_abi_result = "Severe"
                                elif float(right_abi) >= float(
                                    abi_moderate[0]
                                ) and float(right_abi) <= float(abi_moderate[1]):
                                    right_abi_result = "Moderate"
                                elif float(right_abi) >= float(abi_mild[0]) and float(
                                    right_abi
                                ) <= float(abi_mild[1]):
                                    right_abi_result = "Mild"
                                elif float(right_abi) >= float(0.7) and float(
                                    right_abi
                                ) <= float(1.3):
                            ...
                            vpt_ultra_test_values["right_abi_result"] = [
                                "%.2f" % (right_abi),
                                right_abi_result,
                            ]
                    ...
                    if tx_test_entries[21].KEY_VALUE and tx_test_entries[23].KEY_VALUE:
                        if (
                            tx_test_entries[21].KEY_VALUE.isalpha()
                            and tx_test_entries[23].KEY_VALUE.isalpha()
                        ):
                            vpt_ultra_test_values["left_abi_result"] = ["No Data", ""]
                        else:
                            if float(tx_test_entries[21].KEY_VALUE) and float(
                                tx_test_entries[23].KEY_VALUE
                            ):
                                left_abi = round(
                                    float(
                                        float(tx_test_entries[21].KEY_VALUE)
                                        / float(tx_test_entries[23].KEY_VALUE)
                                    ),
                                    2,
                                )
                                if float(left_abi) >= float(abi_severe[0]) and float(
                                    left_abi
                                ) <= float(abi_severe[1]):
                                    left_abi_result = "Severe"
                                elif float(left_abi) >= float(
                                    abi_moderate[0]
                                ) and float(left_abi) <= float(abi_moderate[1]):
                                    left_abi_result = "Moderate"
                                elif float(left_abi) >= float(abi_mild[0]) and float(
                                    left_abi
                                ) <= float(abi_mild[1]):
                                    left_abi_result = "Mild"
                                elif float(left_abi) >= float(0.7) and float(
                                    left_abi
                                ) <= float(1.3):
                            ...
                            vpt_ultra_test_values["left_abi_result"] = [
                                "%.2f" % (left_abi),
                                left_abi_result,
                            ]
                ...
                for medical_test_value in medical_tests:
                    ...
                    if right_count != 0 and right_avg:
                        ...
                        if float(right_avg) >= float(normal[0]) and float(
                            ...
                            right_hand_result = "Not in Range"
                        vpt_ultra_test_values["right_hand_result"] = [
                            "%.1f" % (right_avg),
                            right_hand_result,
                        ]
                    else:
                    ...
                    if left_count != 0 and left_avg:
                        ...
                        if float(left_avg) >= float(normal[0]) and float(
                            ...
                            left_hand_result = "Not in Range"
                        vpt_ultra_test_values["left_hand_result"] = [
                            "%.1f" % (left_avg),
                            left_hand_result,
                        ]
                    else:
            ...
            for medical_test_value in medical_tests:
                ...
                if other_count != 0 and other_avg:
                    ...
                    if float(other_avg) >= float(normal[0]) and float(
                        ...
                        other_result = "Normal"
                    elif float(other_avg) >= float(mild[0]) and float(
                        other_avg
                    ) <= float(mild[1]):
                        other_result = "Mild"
                    elif float(other_avg) >= float(severe[0]) and float(
                        other_avg
                    ) <= float(severe[1]):
                        other_result = "Severe"
                    elif float(other_avg) >= float(moderate[0]) and float(
                        other_avg
                    ) <= float(moderate[1]):
                        ...
                        other_result = "Not in Range"
                    vpt_ultra_test_values["other_result"] = [
                        "%.1f" % (other_avg),
                        other_result,
                    ]
                else:
                ...
                for medical_test_value in medical_tests:
                    ...
                    if right_count != 0 and right_avg:
                        ...
                        if float(right_avg) >= float(normal[0]) and float(
                            ...
                            right_foot_result = "Not in Range"
                        vpt_test_values["right_foot_result"] = [
                            "%.1f" % (right_avg),
                            right_foot_result,
                        ]
                    else:
                    ...
                    if left_count != 0 and left_avg:
                        ...
                        if float(left_avg) >= float(normal[0]) and float(
                            ...
                            left_foot_result = "Not in Range"
                        vpt_test_values["left_foot_result"] = [
                            "%.1f" % (left_avg),
                            left_foot_result,
                        ]
                    else:
                    ...
                    if tx_test_entries[20].KEY_VALUE and tx_test_entries[22].KEY_VALUE:
                        if (
                            ...
                            if float(tx_test_entries[20].KEY_VALUE) and float(
                                ...
                                right_abi = 0.0
                                right_abi_result = "No Data"
                            vpt_test_values["right_abi_result"] = [
                                "%.2f" % (right_abi),
                                right_abi_result,
                            ]
                    else:
                    ...
                    if tx_test_entries[21].KEY_VALUE and tx_test_entries[23].KEY_VALUE:
                        if (
                            ...
                            if float(tx_test_entries[21].KEY_VALUE) and float(
                                ...
                                left_abi = 0.0
                                left_abi_result = "No Data"
                            vpt_test_values["left_abi_result"] = [
                                "%.2f" % (left_abi),
                                left_abi_result,
                            ]
                ...
                for medical_test_value in medical_tests:
                    ...
                    if right_count != 0 and right_avg:
                        ...
                        if float(right_avg) >= float(normal[0]) and float(
                            ...
                            right_hand_result = "Not in Range"
                        vpt_test_values["right_hand_result"] = [
                            "%.1f" % (right_avg),
                            right_hand_result,
                        ]
                    else:
                    ...
                    if left_count != 0 and left_avg:
                        ...
                        if float(left_avg) >= float(normal[0]) and float(
                            ...
                            left_hand_result = "Not in Range"
                        vpt_test_values["left_hand_result"] = [
                            "%.1f" % (left_avg),
                            left_hand_result,
                        ]
                    else:
                    ...
                    if tx_test_entries[18].KEY_VALUE and tx_test_entries[20].KEY_VALUE:
                        if (
                            tx_test_entries[18].KEY_VALUE.isalpha()
                            or tx_test_entries[20].KEY_VALUE.isalpha()
                        ):
                            vpt_test_values["right_abi_result"] = ["No Data", ""]
                        else:
                            right_abi = round(
                                float(
                                    float(tx_test_entries[18].KEY_VALUE)
                                    / float(tx_test_entries[20].KEY_VALUE)
                                ),
                                1,
                            )
                            if float(right_abi) >= float(abi_severe[0]) and float(
                                ...
                                right_abi_result = "Severe"
                            elif float(right_abi) >= float(abi_moderate[0]) and float(
                                right_abi
                            ) <= float(abi_moderate[1]):
                                right_abi_result = "Moderate"
                            elif float(right_abi) >= float(abi_mild[0]) and float(
                                right_abi
                            ) <= float(abi_mild[1]):
                    ...
                    if tx_test_entries[19].KEY_VALUE and tx_test_entries[21].KEY_VALUE:
                        if (
                            tx_test_entries[19].KEY_VALUE.isalpha()
                            and tx_test_entries[21].KEY_VALUE.isalpha()
                        ):
                            vpt_test_values["left_abi_result"] = ["No Data", ""]
                        else:
                            left_abi = round(
                                float(
                                    float(tx_test_entries[19].KEY_VALUE)
                                    / float(tx_test_entries[21].KEY_VALUE)
                                ),
                                1,
                            )
                            if float(left_abi) >= float(abi_severe[0]) and float(
                                ...
                                left_abi_result = "Severe"
                            elif float(left_abi) >= float(abi_moderate[0]) and float(
                                left_abi
                            ) <= float(abi_moderate[1]):
                                left_abi_result = "Moderate"
                            elif float(left_abi) >= float(abi_mild[0]) and float(
    ...
    try:
        ...
        if report_uid:
            ...
            if not password:
                password = settings.EMAIL_HOST_PASSWORD
            if (
                TX_ACCOUNT.objects.filter(DATAMODE="A").exists()
                and reports.MEDICALTEST.PATIENT.PATIENT_EMAIL
            ):
                ...
                if reports.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.CODE == "APP-01":
                    ...
                    message = (
                        "Hospital Name : %s<br />Test Name : Doppler Test<br />"
                        % (hospital_info.BUSINESS_NAME)
                    )
                elif (
                    reports.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.CODE == "APP-02"
                    or reports.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.CODE
                    == "APP-03"
                ):
                    if (
                        reports.MEDICALTEST.MEDICALTESTMASTER.ORGAN_TYPE.lower()
                        == "other"
                    ):
                        subject = "Poly Neuropathy Test - VPT"
                        message = (
                            "Hospital Name : %s<br />Test Name : Poly Neuropathy Test - VPT <br />"
                            % (hospital_info.BUSINESS_NAME)
                        )
                    elif (
                        reports.MEDICALTEST.MEDICALTESTMASTER.ORGAN_TYPE.lower()
                        ...
                        message = (
                            "Hospital Name : %s<br />Test Name : Hand Sensory Test - VPT <br />"
                            % (hospital_info.BUSINESS_NAME)
                        )
                    else:
                        subject = "Diabetic Neuropathy Test - VPT"
                        message = (
                            "Hospital Name : %s<br />Test Name : Diabetic Neuropathy Test - VPT <br />"
                            % (hospital_info.BUSINESS_NAME)
                        )
                    ...
                    if (
                        ...
                        message = (
                            "Hospital Name : %s<br />Test Name : Poly Neuropathy Test - HCP <br />"
                            % (hospital_info.BUSINESS_NAME)
                        )
                    elif (
                        reports.MEDICALTEST.MEDICALTESTMASTER.ORGAN_TYPE.lower()
                        ...
                        message = (
                            "Hospital Name : %s<br />Test Name : Hand Sensory Test - HCP <br />"
                            % (hospital_info.BUSINESS_NAME)
                        )
                    else:
                        subject = "Diabetic Neuropathy Test - HCP"
                        message = (
                            "Hospital Name : %s<br />Test Name : Diabetic Neuropathy Test - HCP <br />"
                            % (hospital_info.BUSINESS_NAME)
                        )
                elif reports.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.CODE == "APP-05":
                    subject = "Foot Pressure Test"
                    message = (
                        "Hospital Name : %s<br />Test Name : Foot Pressure Test<br />"
                        % (hospital_info.BUSINESS_NAME)
                    )
                elif reports.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.CODE == "APP-06":
                    subject = "Foot Temperature Test"
                    message = (
                        "Hospital Name : %s<br />Test Name : Foot Temperature Test<br />"
                        % (hospital_info.BUSINESS_NAME)
                    )
                else:
                    subject = "%s Report" % (
                        reports.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.APP_NAME
                    )
                    message = "Hospital Name : %s<br />Test Name : %s<br />" % (
                        hospital_info.BUSINESS_NAME,
                        reports.MEDICALTEST.MEDICALTESTMASTER.MEDICAL_APP.APP_NAME,
                    )
                ...
                if reports.MEDICALTEST.MEDICALTESTMASTER.ORGAN_TYPE:
                    subject = "%s - %s" % (
                        subject,
                        reports.MEDICALTEST.MEDICALTESTMASTER.ORGAN_TYPE.capitalize(),
                    )
                    message = "%sReport For : %s<br />" % (
                        message,
                        reports.MEDICALTEST.MEDICALTESTMASTER.ORGAN_TYPE.capitalize(),
                    )
                ...
                email_msg = EmailMessage(
                    ...
                    message,
                    from_email,
                    [reports.MEDICALTEST.PATIENT.PATIENT_EMAIL],
                    connection=backend,
                )
                ...
                try:
                    ...
                    result, msg = ulo._setmsg(
                        success_msg, "Username and Password not accepted.", False
                    )
                    logger.error(ulo.error_log(request, sys.exc_traceback.tb_lineno, e))
    except urllib.error.URLError as err:
        result, msg = ulo._setmsg(success_msg, err, False)
...
def search_edit_id(request, patient_uid, search_key):
    ...
    try:
        if (
            TX_PATIENTS.objects.exclude(UID=patient_uid)
            .filter(FRIENDLY_UID=search_key.upper())
            .exists()
        ):
            result, msg = ulo._setmsg(success_msg, error_msg, True)
        else:
        ...
        if option["test_code"] and option["test_entry"]:
            ...
            if medical_tests:
                ...
                for medical_test_value in medical_tests:
                    ...
                    if tx_test_entries[0].KEY_VALUE and (
                        tx_test_entries[1].KEY_VALUE or tx_test_entries[2].KEY_VALUE
                    ):
                        if tx_test_entries[1].KEY_VALUE and float(
                            tx_test_entries[0].KEY_VALUE
                        ):
                            if (
                                float(tx_test_entries[1].KEY_VALUE)
                                and tx_test_entries[2].KEY_VALUE
                            ):
                                        ...
                                        float(
                                            float(
                                                float(
                                                    float(tx_test_entries[1].KEY_VALUE)
                                                    + float(
                                                        tx_test_entries[2].KEY_VALUE
                                                    )
                                                )
                                                / 2
                                            )
                                            / float(tx_test_entries[0].KEY_VALUE)
                                        ),
                                        2,
                                    )
                                else:
                                ...
                                if float(tx_test_entries[2].KEY_VALUE):
                                    right_abi = round(
                                        float(
                                            float(tx_test_entries[2].KEY_VALUE)
                                            / float(tx_test_entries[0].KEY_VALUE)
                                        ),
                                        2,
                                    )
                        ...
                        if right_abi:
                            if float(right_abi) >= float(abi_severe[0]) and float(
                                ...
                                right_abi_result = "Mild"
                            elif float(right_abi) >= float(0.9) and float(
                                right_abi
                            ) <= float(1.3):
                                right_abi_result = "Normal"
                            elif float(right_abi) > float(abi_calcified):
                                ...
                                right_abi_result = "Not in Range"
                            doppler_test_values["right_abi_result"] = [
                                "%.2f" % (right_abi),
                                right_abi_result,
                            ]
                        else:
                    ...
                    if tx_test_entries[4].KEY_VALUE and (
                        tx_test_entries[5].KEY_VALUE or tx_test_entries[6].KEY_VALUE
                    ):
                        if tx_test_entries[5].KEY_VALUE and float(
                            tx_test_entries[4].KEY_VALUE
                        ):
                            if (
                                float(tx_test_entries[5].KEY_VALUE)
                                and tx_test_entries[6].KEY_VALUE
                            ):
                                        ...
                                        float(
                                            float(
                                                float(
                                                    float(tx_test_entries[5].KEY_VALUE)
                                                    + float(
                                                        tx_test_entries[6].KEY_VALUE
                                                    )
                                                )
                                                / 2
                                            )
                                            / float(tx_test_entries[4].KEY_VALUE)
                                        ),
                                        2,
                                    )
                                else:
                                ...
                                if float(tx_test_entries[6].KEY_VALUE):
                                    left_abi = round(
                                        float(
                                            float(tx_test_entries[6].KEY_VALUE)
                                            / float(tx_test_entries[4].KEY_VALUE)
                                        ),
                                        2,
                                    )
                        ...
                        if left_abi:
                            if float(left_abi) >= float(abi_severe[0]) and float(
                                ...
                                left_abi_result = "Mild"
                            elif float(left_abi) >= float(0.9) and float(
                                left_abi
                            ) <= float(1.3):
                                left_abi_result = "Normal"
                            elif float(left_abi) > float(abi_calcified):
                                ...
                                left_abi_result = "Not in Range"
                            doppler_test_values["left_abi_result"] = [
                                "%.2f" % (left_abi),
                                left_abi_result,
                            ]
                        else:
                    ...
                    if tx_test_entries[3].KEY_VALUE and tx_test_entries[0].KEY_VALUE:
                        ...
                        # else:
                        if float(tx_test_entries[3].KEY_VALUE) and float(
                            tx_test_entries[0].KEY_VALUE
                        ):
                            right_tbi = round(
                                float(
                                    float(tx_test_entries[3].KEY_VALUE)
                                    / float(tx_test_entries[0].KEY_VALUE)
                                ),
                                2,
                            )
                            ...
                            doppler_test_values["right_tbi_result"] = [
                                "%.2f" % (right_tbi),
                                right_tbi_result,
                            ]
                        else:
                    ...
                    if tx_test_entries[7].KEY_VALUE and tx_test_entries[4].KEY_VALUE:
                        ...
                        # else:
                        if float(tx_test_entries[7].KEY_VALUE) and float(
                            tx_test_entries[4].KEY_VALUE
                        ):
                            left_tbi = round(
                                float(
                                    float(tx_test_entries[7].KEY_VALUE)
                                    / float(tx_test_entries[4].KEY_VALUE)
                                ),
                                2,
                            )
                            ...
                            doppler_test_values["left_tbi_result"] = [
                                "%.2f" % (left_tbi),
                                left_tbi_result,
                            ]
                        else:
...
def doppler_graphical(request, option):
    ...
    try:
        ...
        previous_graphical_report = TX_MEDICALTESTREPORTS.objects.filter(
            MEDICALTEST__MEDICALTESTMASTER__CODE=medical_test_type.CODE, DATAMODE="A"
        ).first()
        if previous_graphical_report:
            medical_test["previous_graphical_report"] = (
                previous_graphical_report.TEST_TYPE
            )
        else:
            medical_test["previous_graphical_report"] = ""
        medical_test_fields = MA_MEDICALTESTFIELDS.objects.filter(
        ...
        if option["test_code"] and option["test_entry"]:
            ...
            if medical_tests:
                ...
                for medical_test_value in medical_tests:
                    ...
                    if tx_test_entries[0].KEY_VALUE and (
                        tx_test_entries[4].KEY_VALUE or tx_test_entries[5].KEY_VALUE
                    ):
                        # if tx_test_entries[0].KEY_VALUE.isalpha() or (tx_test_entries[4].KEY_VALUE.isalpha() and tx_test_entries[5].KEY_VALUE.isalpha()):
                        #   pass
                        if tx_test_entries[4].KEY_VALUE and float(
                            tx_test_entries[0].KEY_VALUE
                        ):
                            if (
                                float(tx_test_entries[4].KEY_VALUE)
                                and tx_test_entries[5].KEY_VALUE
                            ):
                    ...
                    if tx_test_entries[7].KEY_VALUE and (
                        tx_test_entries[11].KEY_VALUE or tx_test_entries[12].KEY_VALUE
                    ):
                        # if tx_test_entries[7].KEY_VALUE.isalpha() or (tx_test_entries[11].KEY_VALUE.isalpha() and tx_test_entries[12].KEY_VALUE.isalpha()):
                        #   pass
                        if tx_test_entries[11].KEY_VALUE and float(
                            tx_test_entries[7].KEY_VALUE
                        ):
                            if (
                                float(tx_test_entries[11].KEY_VALUE)
                                and tx_test_entries[12].KEY_VALUE
                            ):
                                ...
                                if float(tx_test_entries[12].KEY_VALUE):
                                    left_abi = round(
                                        float(
                                            float(tx_test_entries[12].KEY_VALUE)
                                            / float(tx_test_entries[7].KEY_VALUE)
                                        ),
                                        2,
                                    )
                    ...
                    if tx_test_entries[6].KEY_VALUE and tx_test_entries[0].KEY_VALUE:
                        ...
                        #   doppler_test_values['right_tbi_result'] = ["Invalid Data", ""]
                        # else:
                        if float(tx_test_entries[6].KEY_VALUE) and float(
                            tx_test_entries[0].KEY_VALUE
                        ):
                            right_tbi = round(
                                float(
                    ...
                    if tx_test_entries[13].KEY_VALUE and tx_test_entries[7].KEY_VALUE:
                        ...
                        #   doppler_test_values['left_tbi_result'] = ["Invalid Data", ""]
                        # else:
                        if float(tx_test_entries[13].KEY_VALUE) and float(
                            tx_test_entries[7].KEY_VALUE
                        ):
                            left_tbi = round(
                                float(
                                    float(tx_test_entries[13].KEY_VALUE)
                                    / float(tx_test_entries[7].KEY_VALUE)
                                ),
                                2,
                            )
...
def podo_t_map(request, option):
    ...
    try:
        ...
        t_map_fahrenhit_abnormal = str(
            test_interpertation.RANGES["Fahrenhit"]["Abnormal"]
        )
        ...
        if option["test_code"] and option["test_entry"]:
            ...
            if medical_tests:
                ...
                for medical_test_value in medical_tests:
                    ...
                    if (
                        tx_test_entries[6].KEY_VALUE
                        and not tx_test_entries[6].KEY_VALUE.isalpha()
                    ):
                        for tx_test in tx_test_entries[0:6]:
                            if tx_test.KEY_VALUE.isalpha():
                                ...
                                right_result = float(tx_test.KEY_VALUE) - float(
                                    tx_test_entries[6].KEY_VALUE
                                )
                                if float(right_result) <= float(normal):
                                    ...
                                    tx_test.KEY_VALUE_STATUS = "Abnormal"
                                    podo_t_map_test_values[tx_test.KEY_CODE] = [
                                        "%s %s" % ("Right", tx_test.KEY_NAME),
                                        tx_test.KEY_VALUE_STATUS,
                                    ]
                            else:
                                tx_test.KEY_VALUE_STATUS = "No Data"
                            tx_test.save()
                    if (
                        tx_test_entries[13].KEY_VALUE
                        and not tx_test_entries[13].KEY_VALUE.isalpha()
                    ):
                        for tx_test in tx_test_entries[7:13]:
                            if tx_test.KEY_VALUE.isalpha():
                                ...
                                left_result = float(tx_test.KEY_VALUE) - float(
                                    tx_test_entries[13].KEY_VALUE
                                )
...
def podo_t_map_medical_test_entry(request, p_dict):
    ...
    try:
        ...
        if test_code:
            ...
            t_map_fahrenhit_normal = str(
                test_interpertation.RANGES["Fahrenhit"]["Normal"]
            )
            t_map_fahrenhit_abnormal = str(
                test_interpertation.RANGES["Fahrenhit"]["Abnormal"]
            )
            t_map_celsius_normal = str(test_interpertation.RANGES["Celsius"]["Normal"])
            t_map_celsius_abnormal = str(
                test_interpertation.RANGES["Celsius"]["Abnormal"]
            )
...
def doppler_graphical_template(request, p_dict):
    ...
    try:
        ...
        if (
            test_entry
            and TX_MEDICALTESTREPORTS.objects.filter(
                MEDICALTEST__FRIENDLY_UID=test_entry, DATAMODE="A"
            ).exists()
        ):
            test_report = TX_MEDICALTESTREPORTS.objects.filter(
                MEDICALTEST__FRIENDLY_UID=test_entry, DATAMODE="A"
            ).first()
            if notes:
            ...
            if template:
                with open(
                    "%s/Doppler_Graphical_PDF/%s" % (settings.MEDIA_DATA, template),
                    "wb+",
                ) as destination:
                    for chunk in template.chunks():
                        destination.write(chunk)

                filename = "%s/Doppler_Graphical_PDF/%s" % (
                    settings.MEDIA_DATA,
                    template,
                )
                ...
                if test_report.TEST_TYPE == "VASCULAR_SEGMENTAL_STUDY_REPORT_2":
                    pages[0].save(
                        "%s/Doppler_Graphical/%s1.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                        "JPEG",
                    )
                    image_list.append(
                        "%s/Doppler_Graphical/%s1.jpg"
                    ...
                    pdf_image1 = Image.open(
                        "%s/Doppler_Graphical/%s1.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID)
                    )
                    right_pdf_image = pdf_image1.crop((60, 280, 580, 1270))
                    right_pdf_image.save(
                        "%s/Right_Doppler_Graphical/%s1.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                        "JPEG",
                    )
                    image_list.append(
                    ...
                    left_pdf_image.save(
                        "%s/Left_Doppler_Graphical/%s1.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                        "JPEG",
                    )
                    image_list.append(
                    ...
                    right_base64_image = open(
                        "%s/Right_Doppler_Graphical/%s1.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                        "rb",
                    )
                    right_base64_image_read = right_base64_image.read()
                    right_base64_image_encode = base64.b64encode(right_base64_image_read).decode('utf-8')

                    left_base64_image = open(
                        "%s/Left_Doppler_Graphical/%s1.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                        "rb",
                    )
                    left_base64_image_read = left_base64_image.read()
                    left_base64_image_encode = base64.b64encode(left_base64_image_read).decode('utf-8')
                    doppler_pdf_json["right_base64"] = right_base64_image_encode
                    doppler_pdf_json["left_base64"] = left_base64_image_encode
                    if len(pages) > 1:
                        pages[1].save(
                            "%s/Doppler_Graphical/%s2.jpg"
                            % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                            "JPEG",
                        )
                        image_list.append(
                        ...
                        pdf_image2 = Image.open(
                            "%s/Doppler_Graphical/%s2.jpg"
                            % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID)
                        )
                        right_pdf_image = pdf_image2.crop((60, 280, 580, 440))
                        right_pdf_image.save(
                            "%s/Right_Doppler_Graphical/%s2.jpg"
                            % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                            "JPEG",
                        )
                        image_list.append(
                        ...
                        left_pdf_image.save(
                            "%s/Left_Doppler_Graphical/%s2.jpg"
                            % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                            "JPEG",
                        )
                        image_list.append(
                        ...
                        left_base64_image_read = left_base64_image.read()
                        left_base64_image_encode = base64.b64encode(left_base64_image_read).decode('utf-8')
                        doppler_pdf_json["right_brachial_base64"] = (
                            right_base64_image_encode
                        )
                        doppler_pdf_json["left_brachial_base64"] = (
                            left_base64_image_encode
                        )
                elif test_report.TEST_TYPE == "VASCULAR_SEGMENTAL_STUDY_REPORT_1":
                    for page in pages:
                        page.save(
                            "%s/Doppler_Graphical/%s.jpg"
                            % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                            "JPEG",
                        )
                        image_list.append(
                    ...
                    right_pdf_image.save(
                        "%s/Right_Doppler_Graphical/%s.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                        "JPEG",
                    )
                    image_list.append(
                    ...
                    left_pdf_image.save(
                        "%s/Left_Doppler_Graphical/%s.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                        "JPEG",
                    )
                    image_list.append(
                    ...
                    right_pdf_brachial_image.save(
                        "%s/Right_Doppler_Graphical/brachial_%s.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                        "JPEG",
                    )
                    right_pdf_image.save(
                    ...
                    left_pdf_brachial_image.save(
                        "%s/Left_Doppler_Graphical/brachial_%s.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                        "JPEG",
                    )
                    left_pdf_image.save(
                    ...
                    right_brachial_base64_image = open(
                        "%s/Right_Doppler_Graphical/brachial_%s.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                        "rb",
                    )
                    right_brachial_base64_image_read = (
                        right_brachial_base64_image.read()
                    )
                    right_brachial_base64_image_encode = base64.b64encode(right_brachial_base64_image_read).decode('utf-8')
                    right_base64_image = open(
                        "%s/Right_Doppler_Graphical/%s.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                        "rb",
                    ...
                    left_brachial_base64_image = open(
                        "%s/Left_Doppler_Graphical/brachial_%s.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                        "rb",
                    )
                    left_brachial_base64_image_read = left_brachial_base64_image.read()
                    left_brachial_base64_image_encode = base64.b64encode(left_brachial_base64_image_read).decode('utf-8')
                    left_base64_image = open(
                        "%s/Left_Doppler_Graphical/%s.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                        "rb",
                    ...
                    doppler_pdf_json["right_brachial_base64"] = (
                        right_brachial_base64_image_encode
                    )
                    doppler_pdf_json["left_brachial_base64"] = (
                        left_brachial_base64_image_encode
                    )
                    doppler_pdf_json["right_base64"] = right_base64_image_encode
                    doppler_pdf_json["left_base64"] = left_base64_image_encode
                    ...
                    right_pdf_toe_image.save(
                        "%s/Right_Doppler_Graphical/toe_%s.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                        "JPEG",
                    )
                    image_list.append(
                    ...
                    left_pdf_toe_image.save(
                        "%s/Left_Doppler_Graphical/toe_%s.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                        "JPEG",
                    )
                    image_list.append(
                    ...
                    right_base64_image_read = right_base64_image.read()
                    right_base64_image_encode = base64.b64encode(right_base64_image_read).decode('utf-8')
                    right_base64_toe_image = open(
                        "%s/Right_Doppler_Graphical/toe_%s.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                        "rb",
                    )
                    right_base64_toe_image_read = right_base64_toe_image.read()
                    right_base64_toe_image_encode = base64.b64encode(right_base64_toe_image_read).decode('utf-8')
                    ...
                    left_base64_image_read = left_base64_image.read()
                    left_base64_image_encode = base64.b64encode(left_base64_image_read).decode('utf-8')
                    left_base64_toe_image = open(
                        "%s/Left_Doppler_Graphical/toe_%s.jpg"
                        % (settings.MEDIA_ROOT, test_report.FRIENDLY_UID),
                        "rb",
                    )
                    left_base64_toe_image_read = left_base64_toe_image.read()
                    left_base64_toe_image_encode = base64.b64encode(left_base64_toe_image_read).decode('utf-8')
...
def kodys_can(request, option):
    ...
    try:
        ...
        medical_test_types = MA_MEDICALTESTMASTER.objects.filter(
            MEDICAL_APP=kodys_apps, DATAMODE="A"
        ).order_by("pk")
        medical_test_fields = MA_MEDICALTESTFIELDS.objects.filter(
            MEDICALTESTMASTER=medical_test_type, DATAMODE="A"
        ...
        medical_test["medical_device"] = medical_device
        medical_test["device_details"] = device_details
        instruction = MA_MEDICALTESTINSTRUCTION.objects.get(
            MEDICALTESTMASTER__CODE=medical_test_type.CODE,
            INSTRUCTION_TYPE="PROCEDURE",
            DATAMODE="A",
        )
        medical_test["instruction"] = json.loads(instruction.INSTRUCTION_CONTENT)
        prodecure = MA_MEDICALTESTINSTRUCTION.objects.get(
            MEDICALTESTMASTER__CODE=medical_test_type.CODE,
        ...
        if option["test_code"] and option["test_entry"]:
            ...
            prodecure = MA_MEDICALTESTINSTRUCTION.objects.get(
                MEDICALTESTMASTER__CODE=next_procedure_code,
                INSTRUCTION_TYPE="HELP",
                DATAMODE="A",
            )
            ...
            if tx_test_entries:
                working_data = dict()
                measures = dict()
                qrs_detector = qrs_detect.QRSDetectorOffline(
                    ecg_data=raw_data,
                    verbose=True,
                    log_data=False,
                    plot_data=False,
                ...
                results_td = td.time_domain(
                    nni=working_data["RR_list_cor"], plot=False, show=False
                )
            ...
            if option["test_code"] == "TM-20":
                if tx_test_entries:
                    ...
                    heart_rate = [
                        int(raw_data[i + 1].strip())
                        for i in range(0, len(raw_data))
                        if raw_data[i]
                        if int(raw_data[i].strip()) == 254
                        if int(raw_data[i + 1].strip())
                    ]
                    ...
                    if working_data["RR_list_cor"]:
                        if working_data["RR_list_cor"][0] < 400:
                            ...
                            try:
                                working_data["peaklist_cor"] = working_data[
                                    "peaklist_cor"
                                ].tolist()
                    ...
                    breathing_dict["heart_rate_count_end"] = (
                        int(max(heart_rate_count)) + 5
                    )
                    if (
                        min(working_data["RR_list_cor"]) != 0
                        or min(working_data["RR_list_cor"]) != 0.0
                    ):
                        breathing_dict["rr_interval_start"] = (
                            int(min(working_data["RR_list_cor"])) - 100
                        )
                    else:
                        breathing_dict["rr_interval_start"] = 0
                    breathing_dict["rr_interval_end"] = (
                        int(max(working_data["RR_list_cor"])) + 100
                    )
                    ...
                    if len(test_value) >= 5000:
                        ...
                        avg_deep_breath2 = deep_breathing_rr_calculation(
                            request, context
                        )
                        hr_max = round(60000 / avg_deep_breath2["nni_max"], 2)
                        hr_min = round(60000 / avg_deep_breath2["nni_min"], 2)
                        avg_deep_breath_diff.append(
                            avg_deep_breath2["nni_max"] - avg_deep_breath2["nni_min"]
                        )
                        dbd_list.append(abs(hr_max - hr_min))
                    ...
                    if len(test_value) >= 10000:
                        ...
                        avg_deep_breath4 = deep_breathing_rr_calculation(
                            request, context
                        )
                        hr_max = round(60000 / avg_deep_breath4["nni_max"], 2)
                        hr_min = round(60000 / avg_deep_breath4["nni_min"], 2)
                        avg_deep_breath_diff.append(
                            avg_deep_breath4["nni_max"] - avg_deep_breath4["nni_min"]
                        )
                        dbd_list.append(abs(hr_max - hr_min))
                    ...
                    if len(test_value) >= 12500:
                        ...
                        avg_deep_breath5 = deep_breathing_rr_calculation(
                            request, context
                        )
                        hr_max = round(60000 / avg_deep_breath5["nni_max"], 2)
                        hr_min = round(60000 / avg_deep_breath5["nni_min"], 2)
                        avg_deep_breath_diff.append(
                            avg_deep_breath5["nni_max"] - avg_deep_breath5["nni_min"]
                        )
                        dbd_list.append(abs(hr_max - hr_min))
                    ...
                    if len(test_value) >= 15000:
                        context = {"test_value": test_value[12501:13750]}
                        ...
                        context = {"test_value": test_value[12501:15000]}
                        avg_deep_breath6 = deep_breathing_rr_calculation(
                            request, context
                        )
                        hr_max = round(60000 / avg_deep_breath6["nni_max"], 2)
                        hr_min = round(60000 / avg_deep_breath6["nni_min"], 2)
                        avg_deep_breath_diff.append(
                            avg_deep_breath6["nni_max"] - avg_deep_breath6["nni_min"]
                        )
                        dbd_list.append(abs(hr_max - hr_min))
                    ...
                    if avg_deep_breath_result and (
                        avg_expiration_max_result or avg_inspiration_min_result
                    ):
                        breathing_dict["RSA"] = round(
                            avg_deep_breath_result / breathing_dict["rr_mean"], 2
                        )

                    if avg_expiration_max_result and avg_inspiration_min_result:
                        breathing_dict["ratio"] = round(
                            float(avg_expiration_max_result)
                            / float(avg_inspiration_min_result),
                            2,
                        )

                    normal = (
                        test_interpertation.RANGES["Normal"]
                        .replace(">", "")
                        .replace("=", "")
                    )
                    ab_normal = (
                        test_interpertation.RANGES["Abnormal"]
                        .replace("<", "")
                        .replace("=", "")
                    )
                    ...
                    elif int(hr_diff) >= int(border[0]) and int(hr_diff) <= int(
                        border[1]
                    ):
                ...
                if tx_test_entries:
                    ...
                    heart_rate_count = [heart_rate.count(i) for i in bpm_list]
                    standing_dict["heart_rate_count_end"] = (
                        int(max(heart_rate_count)) + 5
                    )
                    if (
                        min(working_data["RR_list_cor"]) != 0
                        ...
                        standing_dict["rr_interval_start"] = (
                            int(min(working_data["RR_list_cor"])) - 100
                        )
                    else:
                        standing_dict["rr_interval_start"] = 0
                    standing_dict["rr_interval_end"] = (
                        int(max(working_data["RR_list_cor"])) + 100
                    )
                    if len(working_data["RR_list_cor"]) >= 16:
                        standing_dict["15th_min"] = round(
                            min(working_data["RR_list_cor"][12:17]), 2
                        )
                        if len(working_data["RR_list_cor"]) >= 31:
                            standing_dict["30th_max"] = round(
                                max(working_data["RR_list_cor"][27:32]), 2
                            )
                            standing_dict["ratio"] = round(
                                float(max(working_data["RR_list_cor"][27:32]))
                                / float(min(working_data["RR_list_cor"][12:17])),
                                2,
                            )
                        else:
                    ...
                    elif float(standing_dict["ratio"]) >= float(border[0]) and float(
                        standing_dict["ratio"]
                    ) <= float(border[1]):
                    ...
                    standing_dict["supine_peaklist"] = list(
                        working_data["peaklist_cor"]
                    )
                ...
                if tx_test_entries:
                    ...
                    heart_rate_count = [heart_rate.count(i) for i in bpm_list]
                    valsalva_dict["heart_rate_count_end"] = (
                        int(max(heart_rate_count)) + 5
                    )
                    if (
                        min(working_data["RR_list_cor"]) != 0
                        ...
                        valsalva_dict["rr_interval_start"] = (
                            int(min(working_data["RR_list_cor"])) - 100
                        )
                    else:
                        valsalva_dict["rr_interval_start"] = 0
                    valsalva_dict["rr_interval_end"] = (
                        int(max(working_data["RR_list_cor"])) + 100
                    )
                    ...
                    if maneuver_min["nni_min"] and maneuver_max["nni_max"]:
                        valsalva_dict["ratio"] = round(
                            float(maneuver_max["nni_max"])
                            / float(maneuver_min["nni_min"]),
                            2,
                        )
                ...
                if tx_test_entries:
                    ...
                    heart_rate_count = [heart_rate.count(i) for i in bpm_list]
                    resting_dict["heart_rate_count_end"] = (
                        int(max(heart_rate_count)) + 5
                    )
                    if (
                        min(working_data["RR_list_cor"]) != 0
                        ...
                        resting_dict["rr_interval_start"] = (
                            int(min(working_data["RR_list_cor"])) - 100
                        )
                    else:
                        resting_dict["rr_interval_start"] = 0
                    resting_dict["rr_interval_end"] = (
                        int(max(working_data["RR_list_cor"])) + 100
                    )
                    normal_rr = test_interpertation.RANGES["Normal_rr"].split("-")
                    ab_normal_rr = (
                        test_interpertation.RANGES["Abnormal_rr"]
                        .replace("<", "")
                        .replace(">", "")
                        .split("&")
                    )
                    ...
                    ab_normal_hr = (
                        test_interpertation.RANGES["Abnormal_hr"]
                        .replace("<", "")
                        .replace(">", "")
                        .split("&")
                    )
                    rr_mean = float(rr_mean / 1000)
                    if float(rr_mean) >= float(normal_rr[0]) and float(
                        rr_mean
                    ) <= float(normal_rr[1]):
                        resting_dict["final_rr_result"] = "Normal"
                    elif float(rr_mean) < float(ab_normal_rr[0]) or float(
                        rr_mean
                    ) > float(ab_normal_rr[1]):
                        resting_dict["final_rr_result"] = "Abnormal"
                    else:
                        resting_dict["final_rr_result"] = "Not in Range"

                    if int(measures["bpm"]) >= int(normal_hr[0]) and int(
                        measures["bpm"]
                    ) <= int(normal_hr[1]):
                        resting_dict["final_hr_result"] = "Normal"
                    elif int(measures["bpm"]) < int(ab_normal_hr[0]) or int(
                        measures["bpm"]
                    ) > int(ab_normal_hr[1]):
    ...
    try:
        ...
        if option["test_code"] and option["test_entry"]:
            ...
            if option["test_code"] == "TM-23":
                ...
                if (
                    tx_test_entries
                    and tx_test_entries[0].KEY_VALUE
                    and tx_test_entries[6].KEY_VALUE
                ):
                    postural_dict["fall_in_systolic"] = int(
                        tx_test_entries[0].KEY_VALUE
                    ) - int(tx_test_entries[6].KEY_VALUE)
                    postural_dict["supine_bp"] = "%d / %d" % (
                        int(tx_test_entries[0].KEY_VALUE),
                        int(tx_test_entries[1].KEY_VALUE),
                    )
                    postural_dict["standing_bp"] = "%d / %d" % (
                        int(tx_test_entries[3].KEY_VALUE),
                        int(tx_test_entries[4].KEY_VALUE),
                    )
                    postural_dict["after_1_min_bp"] = "%d / %d" % (
                        int(tx_test_entries[6].KEY_VALUE),
                        int(tx_test_entries[7].KEY_VALUE),
                    )
                    normal = (
                        test_interpertation.RANGES["Normal"]
                        .replace("<", "")
                    ...
                    elif int(postural_dict["fall_in_systolic"]) >= int(
                        border[0]
                    ) and int(postural_dict["fall_in_systolic"]) <= int(border[1]):
                ...
                if (
                    tx_test_entries
                    and tx_test_entries[1].KEY_VALUE
                    and tx_test_entries[4].KEY_VALUE
                ):
                    sustained_dict["increase_in_diasystolic"] = int(
                        tx_test_entries[4].KEY_VALUE
                    ) - int(tx_test_entries[1].KEY_VALUE)
                    sustained_dict["before_bp"] = "%d / %d" % (
                        int(tx_test_entries[0].KEY_VALUE),
                        int(tx_test_entries[1].KEY_VALUE),
                    )
                    sustained_dict["after_bp"] = "%d / %d" % (
                        int(tx_test_entries[3].KEY_VALUE),
                        int(tx_test_entries[4].KEY_VALUE),
                    )
                    normal = (
                    ...
                    elif int(sustained_dict["increase_in_diasystolic"]) >= int(
                        border[0]
                    ) and int(sustained_dict["increase_in_diasystolic"]) <= int(
                        border[1]
                    ):
                ...
                if test_report.TEST_TYPE == "parasympathetic_test":
                    para_sympathetic_list = [
                        medical_test_result["resting"]["final_hr_result"],
                        medical_test_result["deep_breathing_time"]["final_result"],
                        medical_test_result["ecg_standing_time"]["final_result"],
                        medical_test_result["ecg_valsalva_time"]["final_result"],
                    ]
                    sympathetic_list = [
                        medical_test_result["ecg_postural"]["final_postural_result"],
                        medical_test_result["ecg_sustained"]["final_sustained_result"],
                    ]
                    para_sympathetic_count = para_sympathetic_list.count("Normal")
                    para_sympathetic_border_count = para_sympathetic_list.count(
                        "Border"
                    )
                    para_sympathetic_abnormal_count = para_sympathetic_list.count(
                        "Abnormal"
                    )
                    ...
                    if sympathetic_count == 2:
                        ...
                        sympathetic_abnormal_count == 1
                        and sympathetic_border_count == 1
                    ):
                    ...
                    if (
                        medical_test_result["para_sympathetic"] == "Normal"
                        and medical_test_result["sympathetic"] == "Normal"
                    ):
        ...
        if option["test_code"] and option["test_entry"]:
            ...
            if tx_test_entries:
                ...
                if (
                    ...
                    hrv_dict["rr_interval_start"] = (
                        int(min(working_data["RR_list_cor"])) - 100
                    )
                else:
                    hrv_dict["rr_interval_start"] = 0
                hrv_dict["rr_interval_end"] = (
                    int(max(working_data["RR_list_cor"])) + 100
                )
                results_td = td.time_domain(
                    nni=working_data["RR_list_cor"], plot=False, show=False
                ...
                cv = (
                    float(round(results_td["sdnn"], 5))
                    / float(round(results_td["nni_mean"], 5))
                ) * 100
                ...
                print(working_data["RR_list_cor"])
                print(
                    "---------------------------------------------------------------------------"
                )

                result_welch, result_frequencies, result_power = fd.welch_psd(
                ...
                for i in range(0, len(result_frequencies)):
                    fft_graph.append(
                        (round(result_frequencies[i], 5), "%.10f" % (result_power[i]))
                    )
                ...
                hrv_dict["log_total_power"] = round(
                    float(np.log(result_welch["fft_total"])), 5
                )
                hrv_dict["log_ratio"] = round(
                    float(np.log(result_welch["fft_ratio"])), 5
                )

                try:
                    fft_spectrum_graph = [
                        float(
                            float(hrv_dict["vlf_power"]) / float(hrv_dict["vlf_peak"])
                        ),
                        float(float(hrv_dict["lf_power"]) / float(hrv_dict["lf_peak"])),
                        float(float(hrv_dict["hf_power"]) / float(hrv_dict["hf_peak"])),
                    ]
                    if fft_spectrum_graph:
                        hrv_dict["fft_spectrum_max"] = max(fft_spectrum_graph) + 100
                        hrv_dict["fft_spectrum_graph"] = [
                            (hrv_dict["vlf_peak"], fft_spectrum_graph[0]),
                            (hrv_dict["lf_peak"], fft_spectrum_graph[1]),
                            (hrv_dict["hf_peak"], fft_spectrum_graph[2]),
                        ]
                except Exception as e:
                    print(e)
        ...
        if option["test_code"] and option["test_entry"]:
            ...
            if medical_tests:
                ...
                for medical_test_value in medical_tests:
                    ...
                    for tx_test in tx_test_entries:
                        podo_i_mat_test_values[tx_test.KEY_NAME.replace(" ", "_")] = (
                            tx_test.KEY_VALUE
                        )
...
def podo_i_mat_foot_convert(request, p_dict):
    ...
    try:
        ...
        if podo_i_mat_image:
            if side.lower() == "right":
                with open(
                    "%s/Right_I_Mat/%s" % (settings.MEDIA_ROOT, podo_i_mat_image), "wb+"
                ) as destination:
                    for chunk in podo_i_mat_image.chunks():
                        destination.write(chunk)
                uploaded_podo_i_mat_image = "%s/Right_I_Mat/%s" % (
                    settings.MEDIA_ROOT,
                    podo_i_mat_image,
                )
            else:
                with open(
                    "%s/Left_I_Mat/%s" % (settings.MEDIA_ROOT, podo_i_mat_image), "wb+"
                ) as destination:
                    for chunk in podo_i_mat_image.chunks():
                        destination.write(chunk)
                uploaded_podo_i_mat_image = "%s/Left_I_Mat/%s" % (
                    settings.MEDIA_ROOT,
                    podo_i_mat_image,
                )
            if uploaded_podo_i_mat_image:
                img = Image.open(uploaded_podo_i_mat_image)
                gray = img.convert("L")

                converted_podo_i_mat_image = uploaded_podo_i_mat_image.replace(
                    podo_i_mat_image.name, "converted_%s" % (podo_i_mat_image.name)
                )

                avging = gray.filter(ImageFilter.BoxBlur(5))
                avging.save(converted_podo_i_mat_image)
    ...
    try:
        ...
        if test_code:
            ...
            if TX_MEDICALTESTREPORTS.objects.filter(
                ...
                if (
                    test_report.TEST_TYPE == "All_test"
                    or test_report.TEST_TYPE == "hrv_test"
                ) and test_code == "TM-25":
                    test_report.TEST_STATUS = "COMPLETED"
                    test_report.save()
            else:
    ...
    try:
        ...
        if test_code:
            ...
            if TX_MEDICALTESTREPORTS.objects.filter(
                ...
                if (
                    test_report.TEST_TYPE == "parasympathetic_test"
                    and test_code == "TM-24"
                ):
                    test_report.TEST_STATUS = "COMPLETED"
                    test_report.save()
...
def deep_breathing_rr_calculation(request, options):
    fn = ulo._fn()
    logger.info(ulo.start_log(request, fn))
    success_msg = "Success"
    error_msg = "ERROR"
