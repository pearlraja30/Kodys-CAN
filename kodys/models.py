...
class MA_STATE(models.Model):
    COUNTRY = models.ForeignKey(MA_COUNTRY, on_delete=models.CASCADE)
    NAME = models.CharField(max_length=255, db_index=True)
...
class MA_MEDICALTESTMASTER(models.Model):
    ...
    CODE = models.CharField(max_length=20)
    MEDICAL_APP = models.ForeignKey(MA_MEDICALAPPS, on_delete=models.CASCADE)
...
class MA_MEDICALTESTFIELDS(models.Model):
    MEDICALTESTMASTER = models.ForeignKey(
        MA_MEDICALTESTMASTER, on_delete=models.CASCADE
    )
...
class MA_ACCOUNTROLEPERMISSION(models.Model):
    ACCOUNTROLE = models.ForeignKey(MA_ACCOUNTROLE, on_delete=models.CASCADE)
    ACCOUNTPERMISSION = models.ForeignKey(
        MA_ACCOUNTPERMISSION, on_delete=models.CASCADE
    )
    CREATED_ON = models.DateTimeField(auto_now_add=True)
    UPDATED_ON = models.DateTimeField(auto_now=True)
    DATAMODE = models.CharField(max_length=1, default="A", choices=DATAMODE_CHOICES)
...
class TX_ACCOUNTLICENSE(models.Model):
    ACCOUNT = models.ForeignKey(TX_ACCOUNT, on_delete=models.CASCADE)
...
class TX_APPLICATIONSERVICEREQUESTS(models.Model):
    ACCOUNT = models.ForeignKey(
        TX_ACCOUNT, blank=True, null=True, on_delete=models.CASCADE
    )
...
class TX_HCP(models.Model):
    ...
    ACCOUNTUSER = models.ForeignKey(
        TX_ACCOUNTUSER, blank=True, null=True, on_delete=models.CASCADE
    )
    NAME = models.CharField(max_length=100)
    # USERNAME = models.CharField(max_length=100)
    TYPE = models.CharField(max_length=100, choices=ACCOUNTROLE_CHOICES)
    # SPECIALIZATION = models.ForeignKey(MA_HCP_SPECIALIZATION, on_delete=models.CASCADE)
    ...
    UPDATED_BY = models.ForeignKey(
        User, related_name="%(class)s_updatedby", on_delete=models.CASCADE
    )
    DATAMODE = models.CharField(max_length=1, default="A", choices=DATAMODE_CHOICES)
...
class TX_HCP_SPECIALIZATION(models.Model):
    SPECIALIZATION = models.ForeignKey(MA_HCP_SPECIALIZATION, on_delete=models.CASCADE)
    HCP = models.ForeignKey(TX_HCP, on_delete=models.CASCADE)
    DATAMODE = models.CharField(max_length=1, default="A", choices=DATAMODE_CHOICES)
...
class TX_PATIENTS(models.Model):
    ...
    ADDRESS_LINE_3 = models.TextField(blank=True, null=True)
    CITY = models.CharField(max_length=50, blank=True, null=True)
    STATE = models.ForeignKey(MA_STATE, blank=True, null=True, on_delete=models.CASCADE)
    COUNTRY = models.ForeignKey(
        MA_COUNTRY, blank=True, null=True, on_delete=models.CASCADE
    )
...
class TX_MEDICALTESTS(models.Model):
    ...
    PATIENT = models.ForeignKey(TX_PATIENTS, on_delete=models.CASCADE)
...
class TX_MEDICALTESTENTRIES(models.Model):
    UID = models.CharField(max_length=100)
    MEDICALTEST = models.ForeignKey(TX_MEDICALTESTS, on_delete=models.CASCADE)
    MEDICALTESTFIELDS = models.ForeignKey(
        MA_MEDICALTESTFIELDS, on_delete=models.CASCADE
    )
    KEY_NAME = models.CharField(max_length=100, blank=True, null=True)
    KEY_CODE = models.CharField(max_length=100)
...
class TX_MEDICALTESTREPORTS(models.Model):
    ...
    TEST_STATUS = models.CharField(
        max_length=20, default="INCOMPLETE", choices=TEST_STATUS_CHOICES
    )
    TEMPERATURE_SCALE = models.CharField(
        max_length=20, choices=TEMPERATURE_SCALE_CHOICES, blank=True, null=True
    )
    IMPRESSION_NOTES = models.TextField(blank=True, null=True)
...
class TX_MEDICALDEVICESTATUS(models.Model):
    UID = models.CharField(max_length=100)
    MEDICAL_DEVICE = models.ForeignKey(MA_MEDICALAPPDEVICES, on_delete=models.CASCADE)
    CONNECTION_STATUS = models.CharField(
        max_length=100, choices=CONNECTIONSTATUS_CHOICES
    )
...
class TX_DATABASEBACKUPLOGS(models.Model):
    TYPE = models.CharField(
        max_length=20, default="AUTOMATIC", choices=DATABASEBACKUP_CHOICES
    )
...
class DOPPLER_GRAPHICAL_PDF(models.Model):
    TEST_REPORT = models.ForeignKey(TX_MEDICALTESTREPORTS, on_delete=models.CASCADE)
...
class TX_MASTER_GENERATED_LICENSES(models.Model):
    ...
    def __unicode__(self):
        return "{0} - {1}".format(self.CLIENT_NAME, self.HARDWARE_ID)
