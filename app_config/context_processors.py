...
def app_config(request):
    ...
    if (
        ...
        and TX_ACCOUNTUSER.objects.filter(USER=request.user).exists()
    ):
    ...
    if TX_ACCOUNTUSER.objects.filter(
        ...
        hospital_accountuser = TX_ACCOUNTUSER.objects.get(
            ACCOUNTROLE__NAME="MANAGER", DATAMODE="A"
        )
    ...
    if hospital_info:
        ...
        if hospital_info.BUSINESS_LOGO_PATH:
            ...
            COMMON_INFO["HOSPITAL_LOGO"] = (
                "/site_media/img/png/default_hospital_logo.png"
            )
    ...
    hospital_name_font_settings = MA_APPLICATION_SETTINGS.objects.get(
        KEY_CODE="SET-09", DATAMODE="A"
    )
    hospital_address_font_settings = MA_APPLICATION_SETTINGS.objects.get(
        KEY_CODE="SET-10", DATAMODE="A"
    )
    hospital_number_font_settings = MA_APPLICATION_SETTINGS.objects.get(
